...
run_prompt_lint() {
  ...
  if [[ ! -f "repo_tools/lint-prompts.py" ]]; then
    warn "repo_tools/lint-prompts.py missing; skipping prompt lint"
    return
  fi
  log "running prompt lint (root: ${PROMPT_ROOT})"
  if ! python3 repo_tools/lint-prompts.py --root "${PROMPT_ROOT}"; then
...
run_prompt_version_check() {
  ...
  if [[ ! -f "repo_tools/prompt-version" ]]; then
    warn "repo_tools/prompt-version missing; skipping"
    return
  fi
  log "running prompt-version dry-run (root: ${PROMPT_ROOT})"
  if ! python3 repo_tools/prompt-version bump --root "${PROMPT_ROOT}" --prompts analyst --kind agent --lang ru --part patch --dry-run >/dev/null; then
