...
## Automation
- Run`claude-workflow research --ticket <ticket> --auto --deep-code --call-graph [--reuse-only] [--paths ... --keywords ... --langs ... --graph-langs ... --note ...]`to populate context JSON (call graph covers supported languages; missing tree-sitter will keep it empty with a warning).
...
3. From`code_index`, open key files/symbols; use`call_graph`/`import_graph`(supported languages) and refine in Claude Code: which functions/classes import or call the targets; note nearby tests/contracts.
...
- For empty contexts (baseline), log TODOs: which commands/paths to run (`rg/find/<build-tool>`) to collect the missing data.
