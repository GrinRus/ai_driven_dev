...
## Входные артефакты
- Перечислите файлы/репорты (`aidd/docs/prd/*.md`, `aidd/docs/research/*.md`, `reports/*.json`, slug-hint в `aidd/docs/.active_feature`) и укажите, как команда находит их (например, `rg <ticket>`).
...
- Перечислите хуки/гейты и команды, запускаемые во время выполнения (`claude-workflow set-active-feature`, `claude-workflow research`, `${CLAUDE_PROJECT_DIR}/.claude/hooks/format-and-test.sh`, `<test-runner> <args>`, `rg`).
...
- Добавьте ссылки на шаблоны (`aidd/agents/templates/prompt-agent.md`, `aidd/docs/prompt-playbook.md`) и требования к структуре правок, включая хранение команд и источников данных.
...
## Примеры CLI
- Приведите пример вызова команды/скрипта (например, `/implement ABC-123` или `!bash -lc 'claude-workflow tasks-derive --source qa --ticket ABC-123'`).
