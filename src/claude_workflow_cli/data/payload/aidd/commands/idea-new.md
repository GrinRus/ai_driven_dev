...
## Контекст
`/idea-new` — единый сценарий: фиксирует активный ticket, запускает аналитика, при нехватке контекста автоматически вызывает research, формирует PRD draft и список вопросов. READY ставится только после ответов пользователя и актуального research; команда завершается в состоянии BLOCKED/READY?BLOCKED с вопросами для пользователя.
...
- Недостаточно данных после research — зафиксируй, что уже проверено (paths/keywords/rg), сформируй вопросы; READY не ставь до ответов.
