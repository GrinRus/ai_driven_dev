...
3. Выполни `claude-workflow research --ticket "$1" --auto --deep-code --call-graph [доп. опции]`(при необходимости `--reuse-only`,`--langs`,`--graph-langs`); если проект не JVM, call graph может быть пустым — зафиксируй предупреждение.
...
5. Запусти саб-агента **researcher** (через палитру) с JSON из `aidd/reports/research/$1-context.json`, используй `call_graph`/`import_graph`(Java/Kotlin) и при необходимости расширь связи в Claude Code, обнови отчёт и перенеси рекомендации.
