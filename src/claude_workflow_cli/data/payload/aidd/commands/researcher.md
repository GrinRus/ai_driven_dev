...
- По умолчанию call graph фильтруется по``&lt;ticket&gt;`|<keywords>`и ограничивается N=100 рёбер; полный граф сохраняется отдельно в`aidd/reports/research/`&lt;ticket&gt;`-call-graph-full.json`, а в context попадает focus-версия.
...
4. Запусти саб-агента **researcher** (через палитру) с JSON из`aidd/reports/research/$1-context.json`, используй`call_graph`/`import_graph`(Java/Kotlin) и при необходимости расширь связи в Claude Code, обнови отчёт и перенеси рекомендации.
5. Зафиксируй статус:`reviewed`, если команда согласовала действия;`pending`, если нужны уточнения (пропиши TODO).
6. Убедись, что ссылки на отчёт добавлены в PRD (`## Диалог analyst`) и tasklist; добавь handoff-пункты (`- [ ] Research: ... (source: aidd/reports/research/$1-context.json)`) или выполни`claude-workflow tasks-derive --source research --append --ticket "$1"`.
