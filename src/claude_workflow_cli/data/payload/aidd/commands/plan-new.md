...
- `${CLAUDE_PLUGIN_ROOT:-./aidd}/tools/set_active_stage.py plan` фиксирует стадию `plan` (можно переустановить при пересборке плана).
...
2. Убедись, что PRD имеет `Status: READY` и актуальный `## PRD Review` со статусом READY. Если нет — сначала запусти `/review-prd <ticket>`.
3. Вызови саб-агента **planner**: он создаст `aidd/docs/plan/<ticket>.md` с архитектурой (KISS/YAGNI/DRY/SOLID, паттерны service layer + adapters/ports по умолчанию), reuse-точками, итерациями, DoD и ссылками на модули.
4. Сразу запусти **validator**. Если он сообщает BLOCKED, верни список вопросов пользователю и дождись ответов, затем обнови план и повтори проверку.
5. Перенеси action items и открытые вопросы из `## PRD Review` в план (разделы «Риски», «Открытые вопросы»), а также обнови одноимённый блок в самом PRD.
6. При необходимости разверни пресет `feature-plan`(см.`claude-presets/feature-plan.yaml`), чтобы добавить типовые итерации.
