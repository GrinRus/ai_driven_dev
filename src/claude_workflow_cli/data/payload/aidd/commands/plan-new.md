...
## Контекст
Команда`/plan-new`преобразует утверждённый PRD в технический план (@aidd/docs/plan/`&lt;ticket&gt;`.md) и сразу прогоняет валидацию. После завершения шаг используют`/tasks-new`,`/implement`.

## Входные артефакты
- @aidd/docs/prd/`&lt;ticket&gt;`.prd.md — статус READY и заполненный`## PRD Review`обязательны.
