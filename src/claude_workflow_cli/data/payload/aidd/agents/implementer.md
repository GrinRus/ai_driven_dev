...
- Кратко перечисли, что изменено (файлы/модули), какие команды выполнены (`./gradlew …`,`${CLAUDE_PLUGIN_ROOT:-./aidd}/hooks/format-and-test.sh`,`claude-workflow progress …`), статусы тестов и автотестов; отметь, если использовал`SKIP_AUTO_TESTS`/`TEST_SCOPE`/`FORMAT_ONLY`. Укажи, какие handoff-пункты из`reports/qa|review|research/<ticket>.json`закрыты или перенесены, и какие пути были добавлены в индекс`git add`.
