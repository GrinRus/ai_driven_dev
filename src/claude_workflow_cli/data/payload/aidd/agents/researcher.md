...
## Автоматизация
- Запускай`claude-workflow research --ticket <ticket> --auto --deep-code --call-graph [--reuse-only] [--paths ... --keywords ... --langs ... --graph-langs ...]`для сбора контекста и актуализации JSON-отчёта (call graph строится для поддерживаемых языков; при отсутствии tree-sitter будет пустой с предупреждением).
...
3. По`code_index`открой ключевые файлы/символы, используй`call_graph`/`import_graph`(для поддерживаемых языков) и при необходимости дорасшифруй связи в Claude Code: какие функции/классы вызывают или импортируют целевые модули; отметь соседние тесты/контракты.
...
- Если данных нет (baseline), зафиксируй TODO: что собрать и какими командами (`rg/find/<build-tool>`) проверить, чтобы закрыть риски.
