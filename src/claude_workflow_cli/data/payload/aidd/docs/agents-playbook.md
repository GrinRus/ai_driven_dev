...
- **Особенности:** блокирующие замечания фиксируйте как `Status: blocked` и переносите action items в `aidd/docs/tasklist/<ticket>.md`; проверяйте, что `aidd/docs/research/<ticket>.md` имеет `Status: reviewed`. Без `approved` гейт `gate-workflow` не даст менять код.
...
- `gate-workflow.sh` активен всегда и блокирует `src/**`, пока не готовы PRD, план и `aidd/docs/tasklist/<ticket>.md`. Он также проверяет, что после изменений появились новые `- [x]` (гейт `tasklist_progress`); для handoff допускаются новые `- [ ]` с ссылкой на отчёт при вызове `claude-workflow progress --source handoff`.
