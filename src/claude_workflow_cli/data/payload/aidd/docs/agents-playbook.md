...
> Текущая стадия фиксируется в `aidd/docs/.active_stage` (`idea/research/plan/tasklist/implement/review/qa`); команды обновляют маркер и позволяют возвращаться к любому этапу.
...
| 6 | `/implement <ticket>` | `implementer` | План, `aidd/docs/tasklist/<ticket>.md`, отчёт Researcher | Кодовые изменения + формат/тесты на Stop/SubagentStop при стадии `implement` | Гейты разрешают правки, тесты зелёные |
