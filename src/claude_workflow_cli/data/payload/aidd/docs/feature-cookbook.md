...
исследование завершено, план сформирован и review-plan/prd-review пройдены.
...
## Plan → Review-plan → Review-prd → Implementation
...
- Перед `review-prd` пройдите `/review-plan`: блокеры должны быть закрыты, иначе `tasks-new` и `implement` недоступны.
