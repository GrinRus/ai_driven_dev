...
Кто выставляет: `plan-reviewer` (через `/review-plan`).
...
Кто выставляет: `prd-reviewer` (через `/review-prd`).
...
- Без `Plan READY` и `Plan Review READY` нельзя начинать `review-prd` и `tasks`.
