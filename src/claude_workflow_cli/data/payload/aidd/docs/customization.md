...
1. **Правим только payload.** Любые изменения `.claude/`, `docs/`, `templates/`, скриптов и агентов в root должны приходить из `src/claude_workflow_cli/data/payload`. Перед редактированием разверните содержание в корень командой `scripts/sync-payload.sh --direction=to-root`.
...
3. **Проверяем контрольные суммы.** Запустите `python3 tools/check_payload_sync.py` (или `pre-commit run payload-sync-check`) — он сравнит хэши файлов payload и runtime snapshot. Любое расхождение блокирует CI (`scripts/ci-lint.sh` → GitHub Actions).
...
```json
{
  ...
  "hooks": {
    "PreToolUse": [{ "matcher": "Write|Edit", "hooks": [{ "type": "command", "command": "\"$CLAUDE_PROJECT_DIR\"/.claude/hooks/gate-workflow.sh" }] }],
...
- Вернуть исходные настройки и хуки можно командой `claude-workflow sync` (по умолчанию синхронизирует `.claude/**`; добавьте `--include claude-presets`, чтобы подтянуть пресеты и `--release <tag|latest>`, если нужна версия из GitHub Releases).
...
### `config/gates.json`
- `feature_ticket_source` — путь к файлу с активным ticket (по умолчанию `docs/.active_ticket`); `feature_slug_hint_source` указывает, где хранить slug-хинт (`docs/.active_feature`).
- `prd_review` — описывает, как проверяется раздел `## PRD Review`: ветки, допустимые (`approved`) и блокирующие (`blocked`) статусы, `allow_missing_report` (разрешить отсутствие отчёта `reports/prd/{ticket}.json`), `blocking_severities` (уровни findings, которые блокируют), `report_path` для кастомизации расположения отчёта.
...
1. **Синхронизируйте payload и промпты.** Выполните `scripts/sync-payload.sh --direction=from-root && python3 tools/check_payload_sync.py`, затем скопируйте обновлённые `.claude/agents|commands` и `prompts/en/**`, чтобы описания ролей и allowed-tools отражали новые требования (логирование команд, ссылки на артефакты).
2. **Обновите шаблоны документов.** Сравните свои `docs/prd.template.md`, `docs/tasklist.template.md`, `docs/templates/research-summary.md` с текущими версиями и перенесите секции «Исходные артефакты», «Автоматизация и проверки», «Commands/Reports`. Это гарантирует, что аналитика/исследование/реализация записывают шаги, а не устные договорённости.
...
4. **Добавьте проверки.** Расширьте `scripts/ci-lint.sh`/`pre-commit` кастомными grep'ами (например, запрещайте «Ответ N» вне `docs/prd/*.md`) и убедитесь, что ваши override-команды по-прежнему перечисляют все используемые CLI.
