...
- RU-файлы используются рантаймом Claude: `aidd/agents/*.md`, `aidd/commands/*.md`.
- EN-файлы живут в `aidd/prompts/en/agents/*.md` и `aidd/prompts/en/commands/*.md`.
...
## 4. Рабочий процесс
> Repo-only: инструменты `scripts/prompt-version`, `scripts/lint-prompts.py`, `tools/prompt_diff.py`, `tools/check_payload_sync.py` живут в корне репозитория и не входят в установленный payload.
> `<workflow-root>` — каталог, где лежат `agents/`, `commands/`, `prompts/en` (например, `./aidd` или `src/claude_workflow_cli/data/payload/aidd`).

1. Изменили RU-промпт → запустите `scripts/prompt-version bump --root <workflow-root> --prompts <name> --kind agent|command --lang ru,en --part <major|minor|patch>` (или обновите вручную, если обе локали меняются одновременно).
2. Обновите EN-файл (перевод, ссылки, примеры).
3. Сравните отличия через `tools/prompt_diff.py --root <workflow-root> --kind agent --name <name>`.
4. Запустите `python3 scripts/lint-prompts.py --root <workflow-root>` — убедитесь, что секции, версии и пары совпадают.
...
- Перед релизом убедитесь, что все правки промптов имеют согласованные версии:
  - `python3 scripts/prompt-version bump --root <workflow-root> --lang ru,en --prompts <список> --part patch --dry-run`
  - `python3 scripts/lint-prompts.py --root <workflow-root>`
  - `python3 tools/prompt_diff.py --root <workflow-root> --kind agent --name <name>` (по необходимости)
...
- Используйте `scripts/prompt-release.sh`, чтобы единой командой выполнить:
  1. `scripts/prompt-version bump --root <workflow-root> --prompts all --kind both --lang ru,en --part <…>` (с `--dry-run`, если нужно только проверить).
  ...
  4. `tools/check_payload_sync.py` (включая директорию `aidd/prompts/en/**`).
