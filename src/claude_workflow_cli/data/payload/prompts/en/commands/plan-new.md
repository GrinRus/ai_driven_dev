...
## Context
`/plan-new` transforms an approved PRD into `docs/plan/<ticket>.md` using the planner and validator agents. Run it before `/tasks-new` or any implementation work.
...
2. Call **planner** to generate the plan (architecture, iterations, DoD, references).
...
## Expected Output
- `docs/plan/<ticket>.md` filled with architecture, iterations, DoD, risks, metrics.
