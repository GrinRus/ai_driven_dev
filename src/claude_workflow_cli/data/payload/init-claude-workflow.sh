...
embed_project_dir_in_settings() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    log_info "[dry-run] rewrite hook commands in $settings_path to absolute paths"
    return
  fi
  ...
  local replacements
  replacements="$(
    python3 - "$settings_path" "$ROOT_DIR" <<'PY'
import json
import pathlib
import sys
...
def update_hook_commands(items):
    updated = 0
...
if isinstance(hooks_section, dict):
    for key in ("PreToolUse", "PostToolUse"):
        total += update_hook_commands(hooks_section.get(key))
...
if isinstance(presets, dict):
    for preset in presets.values():
        ...
        if isinstance(preset_hooks, dict):
            for key in ("PreToolUse", "PostToolUse"):
                total += update_hook_commands(preset_hooks.get(key))
