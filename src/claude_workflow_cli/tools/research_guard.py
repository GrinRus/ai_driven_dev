    ...
    *,
    ...
    if not doc_path.exists():
        ...
        raise ResearchValidationError(
            f"BLOCK: нет отчёта Researcher для {ticket} → запустите `claude-workflow research --ticket {ticket}` и оформите docs/research/{ticket}.md"
        )
            ...
            if status == "pending" and settings.allow_pending_baseline:
                ...
                if baseline_phrase and baseline_phrase in doc_text_lower:
                    try:
                        ...
                        raise ResearchValidationError(
                            f"BLOCK: отсутствует {context_path}; выполните claude-workflow research для {ticket}."
                        )
    ...
    if min_paths > 0:
        try:
            ...
            raise ResearchValidationError(
                f"BLOCK: повреждён файл {targets_path}; пересоберите его командой claude-workflow research."
            )
    ...
    if freshness_days:
        ...
        if age_days > int(freshness_days):
            raise ResearchValidationError(
                f"BLOCK: контекст Researcher устарел ({age_days} дней) → обновите claude-workflow research для {ticket}."
            )
