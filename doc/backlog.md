...
- [ ] Автодетект `aidd/` в CLI/analyst-check: если команда запущена из родительского каталога без `docs/`, но с `aidd/docs/`, использовать `./aidd` как root; вывести подсказку про `--target aidd`/рабочий корень вместо `BLOCK: нет PRD`.
- [ ] Проверить все инструкции агентов/команд на вызовы `.claude/hooks/*.sh`: заменить голые пути на `${CLAUDE_PROJECT_DIR}/.claude/hooks/...` (или `${CLAUDE_PLUGIN_ROOT}` для плагинных хуков), чтобы команды и агенты работали из корня репозитория с подпапкой `aidd/`.
