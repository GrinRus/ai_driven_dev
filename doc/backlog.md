...
- [x] `src/claude_workflow_cli/tools/researcher_context.py`, `tools/researcher_context.py`, `src/claude_workflow_cli/data/payload/tools/researcher_context.py`: добавить опциональное построение графа вызовов/импортов. Флаги CLI `--call-graph`/`--graph-engine {auto,none,ts}`, сбор ребёр `caller → callee` (file:line, symbol) и импорт-графа; fallback на текущую эвристику, WARN при отсутствии движка.
...
- [x] Опциональный движок tree-sitter: поддержать авто-режим только для Java/Kotlin (kt/kts/java); остальные языки анализируются без графа. Fallback при отсутствии зависимости; обеспечить graceful degrade в офлайне и явно писать об ограничении языков.
- [x] Тесты: `tests/test_researcher_context.py` + новый фикстур на call graph (kt/java), `tests/test_researcher_call_graph.py` — проверка рёбер, fallback без tree-sitter, негативные кейсы (пустые совпадения); обновить smoke, чтобы проверять наличие `call_graph` в JSON и ссылок в отчёте.
...
- [x] Зависимости: добавить optional extra (например, `call-graph`) в `pyproject.toml` с `tree-sitter`/`tree-sitter-language-pack`, описать установку через `uv tool install ... --extra call-graph`/`pip install ...[call-graph]`, и предусмотреть CI job, который ставит extra и прогоняет e2e тест на реальных `.java/.kt` (skip без extra оставить).
