...
- [ ] `.claude/hooks/gate-qa.sh`, `config/gates.json`, `src/claude_workflow_cli/cli.py`: enforce автозапуск тестов в QA-стадии, проверять наличие записей в tasklist после проверки, добавлять подсказку/hand-off в CLI вывод.
- [ ] `.claude/agents/qa.md`, `prompts/en/agents/qa.md`, payload: обновить мандат/формат ответа — включать список прогнанных тестов, ссылки на логи/репорты, новые чекбоксы/статусы в tasklist для последующих итераций имплементера; sync RU/EN и автотесты.
