...
- [ ] Документация/трешутинг: README/workflow/команды — явное упоминание, что все артефакты лежат под `./aidd/...`; добавить совет “Если `Read(reports/...)` не находит файл — смотрите `aidd/reports/...`, используйте `${CLAUDE_PLUGIN_ROOT:-./aidd}`/`--target aidd`”. *(EN/README/workflow пока без обновлений)*
