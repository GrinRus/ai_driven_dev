...
- [x] Расширить `src/claude_workflow_cli/data/payload/aidd/scripts/lint-prompts.py`: проверка дубликатов ключей фронт‑маттера, запрет неизвестных статусов, поиск `&lt;ticket&gt;`, контроль формата `Checkbox updated` и совпадения `allowed-tools` с инструментами агента.
