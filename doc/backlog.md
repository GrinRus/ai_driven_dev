...
- [ ] `init-claude-workflow.sh`, `scripts/smoke-workflow.sh`: синхронизировать bash-обёртку с новой структурой, покрыть пустую/существующую директорию, гарантировать идентичную логику с CLI и новой раскладкой `aidd/`. _Progress: smoke обновлён — ставит workflow через `claude-workflow init` в `aidd/` и гоняет гейты оттуда; init-скрипт ещё без отдельной ветки под subdir._
