...
## 4. Рабочий процесс
> Repo-only: `repo_tools/prompt-version`, `repo_tools/lint-prompts.py` живут в корне репозитория.

1. Обновите текст промпта.
2. Поднимите версии: `repo_tools/prompt-version bump --root <workflow-root> --prompts <name> --kind agent|command --lang ru --part <major|minor|patch>`.
3. Запустите lint: `python3 repo_tools/lint-prompts.py --root <workflow-root>`.
4. Зафиксируйте изменения в `doc/dev/release-notes.md` и при необходимости в `CHANGELOG.md`.

## 5. Инструменты
- `repo_tools/lint-prompts.py` — валидирует фронт-маттер и обязательные секции.
- `repo_tools/prompt-version bump --root <workflow-root> --prompts analyst --kind agent --lang ru --part minor` — повышает версии. Используйте `--dry-run` для проверки.

## 6. Release checklist
- `python3 repo_tools/prompt-version bump --root <workflow-root> --prompts <list> --kind agent|command --lang ru --part patch --dry-run`
- `python3 repo_tools/lint-prompts.py --root <workflow-root>`
