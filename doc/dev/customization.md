...
3. **Переинициализируйте активные фичи.** Прогоните `claude-workflow research --ticket <ticket> --auto` и `claude-workflow analyst-check --ticket <ticket>` для всех актуальных задач — отчёты и PRD будут соответствовать обновлённым шаблонам. Tasklist заполните новыми секциями `Reports/Commands` вручную или скриптом.
