...
| `repo_tools/` | Smoke/CI и dev-утилиты | dev-only | не используется в runtime |
...
| `tests/` | Юнит/интеграционные тесты | dev-only | CI-only |
| `doc/dev/` | ADR/дизайн/backlog | dev-only | планирование |
...
- Dev-only артефакты (tests/doc/dev) не должны попадать в дистрибутив.
