...
def _render_table(rows: List[Tuple[str, int, int, int]]) -> str:
    if not rows:
        return "No reports found. Run `python3 dev/repo_tools/report_stats.py --write` after reports exist."
...
def main() -> int:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Update dev/doc/reports-format.md between report-stats markers.",
    )
    ...
    if args.write:
        doc_path = root / "dev" / "doc" / "reports-format.md"
