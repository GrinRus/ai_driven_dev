...
run_prompt_lint() {
  ...
  if [[ ! -f "dev/repo_tools/lint-prompts.py" ]]; then
    warn "dev/repo_tools/lint-prompts.py missing; skipping prompt lint"
    return
  fi
  log "running prompt lint (root: ${PROMPT_ROOT})"
  if ! python3 dev/repo_tools/lint-prompts.py --root "${PROMPT_ROOT}"; then
...
run_prompt_version_check() {
  ...
  if [[ ! -f "dev/repo_tools/prompt-version" ]]; then
    warn "dev/repo_tools/prompt-version missing; skipping"
    return
  fi
  log "running prompt-version dry-run (root: ${PROMPT_ROOT})"
  if ! python3 dev/repo_tools/prompt-version bump --root "${PROMPT_ROOT}" --prompts analyst --kind agent --lang ru --part patch --dry-run >/dev/null; then
...
run_markdownlint() {
  ...
  local md_config="${root}/dev/.markdownlint.yaml"
  if [[ -f "$md_config" ]]; then
...
run_python_tests() {
  ...
  if [[ ! -d "dev/tests" ]]; then
    log "dev/tests/ directory not found; skipping python tests"
    return
  fi
  log "running python unittest suite"
  if ! AIDD_PACK_ENFORCE_BUDGET=1 python3 -m unittest discover -s dev/tests -t dev; then
