...
- [x] `dev/doc/workflow.md`: переписать walkthrough под новый цикл, добавить шаги `/idea-new → /plan-new → /tasks-new` и детальные проверки гейтов (workflow/API/DB/tests).
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`: пересобрать разделы «Быстрый старт», «Слэш-команды», чеклист, убрав ссылки на исключённые команды и подчёркивая основной цикл `/idea-new → /plan-new → /tasks-new → /implement → /review`.
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`: сфокусировать описание флоу на `/idea-new → /plan-new → /tasks-new → /implement → /review`, убрать секции про необязательные артефакты.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `README.md`: описать обновлённый порядок `/idea-new → claude-workflow research → analyst`, правила для пустых репозиториев и требования к паттернам.
...
### Документация и обучение
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, `dev/doc/agents-playbook.md`: описать, что `/idea-new` сразу создаёт PRD-шаблон, поэтому гейты больше не просят перезапуск; выделить обязанности агента (заполнить `## Диалог analyst`, снять статус draft).
...
- [ ] `dev/tests/test_cli_entrypoint.py` (новый), `dev/repo_tools/smoke-workflow.sh`, `dev/repo_tools/ci-lint.sh`: добавить проверки, эмулирующие чистую систему (`PYTHONPATH` без `src`, только бинарь `claude-workflow`), и убедиться, что helper корректно находит CLI; при отсутствии бинаря тесты должны давать понятное сообщение и ссылку на команды установки.
  ...
  - `dev/repo_tools/smoke-workflow.sh` перед вызовом helper сбрасывает `PYTHONPATH`, временно скрывает `src` (например, запускается в подпапке без dev-src) и использует `run_cli` для всех команд, чтобы подтвердить zero-touch E2E;
...
- [x] `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `dev/doc/qa-playbook.md`, `README.md`, `README.en.md`: включить обязательный шаг `/qa` (после `/review`) в walkthrough/quick-start, перечислить входы (diff, tasklist QA, логи гейтов), параметры гейта (`CLAUDE_SKIP_QA`, `--only qa`, dry-run), формат отчёта и прогресс-маркеры.
...
- [x] Обновить `dev/repo_tools/smoke-workflow.sh`: сценарий `/idea-new` с авто-analyst+auto-research, подтверждение корневого запуска без `docs/`, готовность после ответов/исправлений.
...
- [x] Покрыть хуки: `dev/tests/test_gate_workflow.py`, `dev/tests/test_gate_tests_hook.py`, `dev/tests/test_gate_qa.py` — убедиться, что пути резолвятся из `aidd/`, нет попыток читать `./docs`, PostToolUse/PreToolUse команды не используют `CLAUDE_PROJECT_DIR`.
...
- [x] Tasklist фокус: добавить `Next 3 checkboxes` и `Handoff inbox` в `src/claude_workflow_cli/data/payload/aidd/docs/tasklist/template.md` и `src/claude_workflow_cli/data/payload/aidd/docs/tasklist/template.md`; обновить `/tasks-new`.
...
- [ ] Обновить документацию об источнике root и запуске CLI: `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/agents-playbook.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md` (правило: хуки и CLI работают от workspace root; `AIDD_ROOT` как override).
- [ ] Добавить тесты на резолвинг root: эмуляция `AIDD_ROOT`, `CLAUDE_PROJECT_DIR`, `CLAUDE_PLUGIN_ROOT` (cache path), запуск hook из чужого cwd; отдельный кейс non‑git — ожидается мягкий exit. Файлы: `dev/tests/test_gate_workflow.py`, `dev/tests/helpers.py`.
...
- [x] Добавить JSONL логи `reports/tests/<ticket>.jsonl` и CLI helper для append, обновить `/status` и `index_sync.py`.
...
- [x] Обновить `/review` и reviewer agent. **Deps:** tasks-derive review. **AC:** после ревью создаётся отчёт, запускается `tasks-derive --source review --append`; tasklist содержит review‑задачи; добавлены tools для CLI‑команд. Файлы: `src/claude_workflow_cli/data/payload/aidd/commands/review.md`, `src/claude_workflow_cli/data/payload/aidd/agents/reviewer.md`.
- [x] Сделать review handoff идемпотентным для повторных запусков. **Deps:** review report + tasks-derive review. **AC:** повторный `/review` не дублирует задачи; задачи маппятся по стабильному `id` и обновляются при изменении текста/рекомендаций; сохраняется история (например, через `Updated:`/`Last seen` в отчёте). Файлы: `src/claude_workflow_cli/cli.py`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/reports-format.md`.
...
- [x] Тесты на повторные запуски QA/Review. **Deps:** идемпотентность handoff. **AC:** двойной `tasks-derive` не добавляет дубликатов, обновляет существующие задачи по id; smoke‑сценарий проверяет повторный `/qa` и `/review`. Файлы: `dev/tests/test_tasks_derive.py`, `dev/repo_tools/smoke-workflow.sh`.
...
- [x] Привязать `/spec-interview` к итерациям плана: вопросы формируются по `iteration_id` и маппятся на секции spec. **AC:** интервью‑лог и spec содержат явные ссылки на `iteration_id`/решения. Файлы: `aidd/commands/spec-interview.md`, `aidd/docs/anchors/spec-interview.md`, payload‑копии.
...
- [x] Зафиксировать политику обязательности spec (UI/API/DATA/E2E → mandatory) в tasklist‑anchor/refiner/`/tasks-new`. **AC:** при trigger‑условиях и отсутствии spec — `Status: BLOCKED` + `/spec-interview`. Файлы: `aidd/docs/anchors/tasklist.md`, `aidd/agents/tasklist-refiner.md`, `aidd/commands/tasks-new.md`, payload‑копии.
...
- [x] Создать пакет runtime `aidd_runtime/` и перенести логику из `src/claude_workflow_cli/*` (research, progress, qa, prd/plan review, tasks-derive, reviewer-tests, status/index, context-gc, set-active-*). **AC:** функциональность доступна из python‑модуля без CLI‑инициализатора.
...
- [x] Добавить `/aidd-init` и runtime‑инициализацию, которая разворачивает `./aidd` из `templates/aidd` без перезаписи пользовательских файлов. **AC:** повторный запуск идемпотентен, `reports/*` содержит `.gitkeep`.
- [x] Обновить хуки/доки на новый init‑путь. **AC:** инструкции в README и workflow указывают `/aidd-init` как обязательный шаг после установки плагина.

### EPIC D — Tests и контроль целостности
- [x] Обновить тесты и smoke‑сценарии под запуск `python3 -m aidd_runtime.cli` и `/aidd-init`. **AC:** существующие тесты остаются; добавлены проверки init и вызовов runtime.
...
- [x] Переписать `README.md`/`README.en.md`/`dev/doc/workflow.md` под поток `/plugin marketplace add` → `/plugin install` → `/aidd-init`. **AC:** нет упоминаний `claude-workflow init/sync/upgrade`.
...
- [x] Определить каноническую команду запуска runtime без `scripts/` и без `src/` (например, `PYTHONPATH="${CLAUDE_PLUGIN_ROOT:-.}" python3 -m aidd_runtime.cli <cmd> ...`) и зафиксировать её в документации. **AC:** единый шаблон запуска описан; в коде используются только новые вызовы.
- [x] Перенести init‑логику из `scripts/aidd_init.py` в runtime (новый модуль или функции в `aidd_runtime`), обновить `/aidd-init` на вызов `python3 -m aidd_runtime.cli init ...` с `PYTHONPATH`. **AC:** `/aidd-init` работает без `scripts/aidd_init.py`; повторный запуск идемпотентен.
- [x] Обновить `commands/`, `agents/`, `hooks/` и `hooks/hooks.json` (включая `aidd/hooks/hooks.json`) на вызовы `python3 -m aidd_runtime.cli ...` с `PYTHONPATH`; синхронизировать `allowed-tools` в командах/агентах. **AC:** `rg "scripts/aidd_"` не находит обращений в runtime‑путях.
- [x] Удалить обёртки и bootstrap + убрать src‑layout: `scripts/aidd_*.py`, `scripts/_aidd_bootstrap.py`, `aidd_runtime/runner.py`, `src/`. **AC:** файлы отсутствуют, сборка не зависит от них.
- [x] Обновить smoke/dev/tests/docs под новый путь запуска и плоский layout (включая `dev/repo_tools/smoke-workflow.sh`, тесты CLI, README/дев‑доки). **AC:** `dev/repo_tools/ci-lint.sh` и `dev/repo_tools/smoke-workflow.sh` проходят, упоминаний `scripts/aidd_*.py` и `PYTHONPATH=.../src` нет.

### EPIC G — Вынос hook‑скриптов в runtime
- [ ] Вынести `hooks/format-and-test.sh` в runtime‑команду (например, `aidd_runtime.cli format-and-test`) и обновить вызовы. **AC:** `hooks/hooks.json`, `dev/repo_tools/smoke-workflow.sh`, `dev/tests/test_format_and_test.py` используют CLI; скрипт либо удалён, либо тонкий shim.
- [ ] Перенести `hooks/gate-workflow.sh` в runtime‑команду `gate-workflow` с сохранением exit‑кодов и сообщений. **AC:** `hooks/hooks.json` и `dev/tests/test_gate_workflow.py`/`dev/tests/test_gate_researcher.py` работают через CLI; документация/гайд обновлены.
- [ ] Перенести `hooks/gate-tests.sh` в runtime‑команду `gate-tests`. **AC:** `hooks/hooks.json`, `dev/tests/test_gate_tests_hook.py`, `dev/repo_tools/smoke-workflow.sh` используют CLI; поведение совпадает с текущим.
- [ ] Перенести `hooks/gate-qa.sh` в runtime‑команду `gate-qa` и сохранить интеграцию с `aidd_runtime.tools.qa_agent`. **AC:** `hooks/hooks.json`, `dev/tests/test_gate_qa.py` используют CLI; `dev/doc/qa-playbook.md` и playbook‑ы отражают новый вызов.
- [ ] Перенести `hooks/lint-deps.sh` в runtime‑команду `lint-deps`. **AC:** `hooks/hooks.json` и `dev/tests/test_post_hook_paths.py` используют CLI; поведение по `deps_allowlist` не меняется.
- [ ] Разобраться с неиспользуемыми `hooks/gate-prd-review.sh`, `hooks/gate-api-contract.sh`, `hooks/gate-db-migration.sh`: удалить или перенести в runtime с явными точками вызова. **AC:** нет «висячих» скриптов без вызовов; тесты/доки синхронизированы.
- [ ] Заменить `hooks/lib.sh` на python‑хелперы и убрать зависимость от shell‑shared кода (или оставить тонкий shim). **AC:** новые runtime‑команды используют единые helper‑функции; shell‑скрипты не зависят от `lib.sh`.

### EPIC H — Вынос dev‑скриптов (repo‑only)
- [ ] Перенести `dev/repo_tools/ci-lint.sh` в python‑раннер (или CLI‑подкоманду) и обновить ссылки в `README.md`, `README.en.md`, `CONTRIBUTING.md`, `dev/doc/release-notes.md`. **AC:** запуск CI‑линта не требует shell‑скрипта; документация указывает новую команду.
- [ ] Перенести `dev/repo_tools/smoke-workflow.sh` в python (например, полноценная реализация `aidd_runtime.cli smoke`) и обновить `aidd_runtime/cli.py`/доки. **AC:** smoke не зависит от bash‑скрипта; `README*` и dev‑доки ссылаются на новый запуск.
