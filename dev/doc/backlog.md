...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`,: пересобрать разделы «Быстрый старт», «Слэш-команды», чеклист, убрав ссылки на исключённые команды и подчёркивая основной цикл `/feature-dev-aidd:idea-new → /feature-dev-aidd:plan-new → /feature-dev-aidd:tasks-new → /feature-dev-aidd:implement → /feature-dev-aidd:review`.
...
- [x] `.claude/commands/api-spec-new.md`: убрать из шаблона, удалить привязку к /api-spec-new во всех гидах (`README.md`, `README.en.md`, `dev/doc/workflow.md`) и из чеклистов.
...
- [x], `dev/doc/workflow.md`, `dev/doc/customization.md`: переписать сценарии, заменив вызовы `/api-spec-new` и `/tests-generate` на прямое использование агентов или ручные шаги.
...
- [x], `dev/doc/workflow.md`: добавить явное пояснение, как работает авто запуск тестов после записи и как временно отключить/расширить его без использования устаревшей команды.
...
- [x], `dev/doc/workflow.md`, `dev/doc/customization.md`: обновить флоу с шагом PRD Review, описать, когда вызывать агента, как интерпретировать вывод и какие поля нужно обновить в PRD/плане; привести пример output.
...
- [x], `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`: обновить флоу — описать, какую информацию приносит Researcher, как читать отчёт и каким образом разработчик подтверждает, что рекомендации применены.
- [x] `dev/doc/workflow.md`,: добавить walkthrough с вызовом Researcher на демо-фиче, демонстрацией найденных участков кода и тем, как отчёт используется в QA/код-ревью.
...
- [x], `dev/doc/workflow.md`: расширить описание роли analyst с пошаговым Q&A-циклом, критерия READY и форматом фиксации ответов.
...
- [x] `dev/doc/workflow.md`, `README.md`, `README.en.md`: обновить схемы и walkthrough, подчёркивая, что tasklist теперь хранится в `docs/tasklist/<ticket>.md`.
...
- [x] `dev/doc/workflow.md`, `dev/tests/test_gate_tests_hook.py`, `dev/repo_tools/smoke-workflow.sh`: зафиксировать новый порядок действий, обновить чеклисты и покрыть сценарий запуска тестов по запросу reviewer/ручному override.
...
- [x] `dev/doc/workflow.md`,: переписать walkthrough, подчёркивая итеративное отмечание прогресса и необходимость ссылаться на обновлённые чекбоксы в ответах агентов.
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`,: переписать walkthrough и примеры команд под модель TICKET-first, описать роль slug-хинта как пользовательского ориентира.
...
- [x], `dev/doc/workflow.md`, `README.md`: описать обновлённый порядок `/feature-dev-aidd:idea-new → claude-workflow research → analyst`, правила для пустых репозиториев и требования к паттернам.
...
### Документация и обучение
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`,: описать, что `/feature-dev-aidd:idea-new` сразу создаёт PRD-шаблон, поэтому гейты больше не просят перезапуск; выделить обязанности агента (заполнить `## Диалог analyst`, снять статус draft).
...
- [x], `dev/doc/workflow.md`, `dev/doc/backlog.md`: переписать правила с упором на «agent-first» модель (агент сам читает файлы и запускает скрипты, вопросы пользователю — крайняя мера), убрать упоминания человеческих ресурсных ограничений и добавить примеры автоматических источников данных.
...
- [x] `dev/doc/workflow.md`,, `README.md`, `README.en.md`: включить обязательный шаг `/feature-dev-aidd:qa` (после `/feature-dev-aidd:review`) в walkthrough/quick-start, перечислить входы (diff, tasklist QA, логи гейтов), параметры гейта (`CLAUDE_SKIP_QA`, `--only qa`, dry-run), формат отчёта и прогресс-маркеры.
...
- [x] Документация (`dev/doc/release-notes.md`, `CHANGELOG.md`): отметить, что backlog не поставляется конечным пользователям, добавить запись в release checklist и guidance по ведению roadmap в репо без включения в payload.
...
- [x], `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, `README.md`, `README.en.md`: описать новый формат работы Researcher (deep-code + reuse), обязательные поля отчёта, примеры интерпретации и как применять рекомендации в план/тасклист; добавить guidance по запуску с фильтрами директорий/языков и явным шагом построения графа на стороне Claude Code.
...
- [x] Документация:, `dev/doc/workflow.md`, `README.md`, `README.en.md`, `dev/doc/feature-cookbook.md` — описать опцию call graph (когда использовать, параметры CLI, ограничения на языки), примеры вывода и влияние на гейты/планирование.
...
- [x] Документация: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `docs/tasklist/template.md` — добавить шаг handoff после отчётов, пример вызова `tasks-derive`, формат маппинга finding → чекбокс и роль implementer в закрытии этих пунктов.
...
- [ ] Документация:, `dev/doc/feature-cookbook.md`, `dev/doc/workflow.md` — добавить таблицу «когда запускать graph vs обычный ресерч», примеры WARN/INSTALL_HINT, troubleshooting для пустого контекста.
...
- [x] Документация (`README*`, `dev/doc/workflow.md`, `aidd/CLAUDE.md`): явное разделение project `.claude/` vs plugin `.claude-plugin/`, примеры подключения marketplace из подпапки `aidd/`, новые пути хуков.
...
### Дока и DX
- [x] Обновить `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/adr/install-subdir-decision.md`, `aidd/`: убрать советы про произвольный `--target`, показать единственную установку `claude-workflow init --target.` → `./aidd`, troubleshooting «CLI не найден/не видит.claude» с ссылкой на фиксированный путь.
...
- [x] Синхронизировать порядок стадий и ссылки на статусы во всех документах: `README.md`, `README.en.md`, `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `AGENTS.md`, `src/claude_workflow_cli/data/payload/aidd/`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/feature-cookbook.md`, `src/claude_workflow_cli/data/payload/aidd/` (ссылки на `sdlc-flow.md` и `status-machine.md`).
...
- [x] Документация процесса: синхронизировать `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, (и payload‑версии) с новым порядком idea → research → plan и раздельными гейтами.
...
- [ ] Обновить контракт команд в `src/claude_workflow_cli/data/payload/aidd/`: добавить `/flow` в матрицу (входы/выходы), описать политику `SlashCommand` и handoff‑правила.
...
- [ ] Обновить документацию об источнике root и запуске CLI: `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `src/claude_workflow_cli/data/payload/aidd/`, `src/claude_workflow_cli/data/payload/aidd/` (правило: хуки и CLI работают от workspace root).
...
- [x] Удалить `prompts/en/**`: убрать `--prompt-locale` из CLI и init‑скрипта, удалить паритет‑проверки из `gate-workflow.sh`, пересмотреть `dev/repo_tools/lint-prompts.py`, обновить `dev/doc/prompt-versioning.md` и, удалить упоминания из README/тестов.
...
- [x] Определить финальное место для `dev/doc/templates/prompts/prompt-agent.md` и `dev/doc/templates/prompts/prompt-command.md` (например, `agents/templates/` и `commands/templates/` или `dev/doc/templates/prompts/`), перенести и обновить `scripts/scaffold_prompt.py`, README.

### Docs: runtime vs dev-only
- [x] Разделить maintainer‑инструкции: вынести repo-only шаги из `aidd/dev/doc/release-notes.md`, `aidd/dev/doc/prompt-versioning.md`, `aidd/` в `dev/doc/**`; в payload оставить user‑facing версии без ссылок на `scripts/*` и `tools/*` из корня.
...
- [x] Определить dev-only местоположение и консолидировать версии: `aidd/`, `aidd/dev/doc/feature-cookbook.md`, `aidd/`, `aidd/dev/doc/migration-aidd.md`, `aidd/`, `aidd/dev/doc/prompt-versioning.md`, `aidd/dev/doc/release-notes.md` → `dev/doc/**` (свести дубли, сохранить историю).
...
- [x] Обновить `dev/doc/customization.md` и: anchors‑first/pack‑first, `AIDD:CONTEXT_PACK`, новые дефолты профиля и формат логов.
...
- [x] Обновить `dev/doc/adr/install-subdir-decision.md`: зафиксировать marketplace‑only и отказ от CLI‑установки. **AC:** ADR синхронизирована с новой моделью.
...
- [x] Обновить документацию и шаблоны: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `templates/aidd/conventions.md`, `templates/aidd/docs/prd/template.md`. **AC:** в docs нет упоминаний `python3 -m` как основного способа вызова.
...
- [x] Убрать runtime‑smoke entrypoint, оставив repo‑smoke в `dev/repo_tools/`. **AC:** в `tools/` нет smoke‑скрипта; runtime‑доки не ссылаются на smoke; dev‑smoke остаётся в `dev/repo_tools/`.
...
- [x] Разделить smoke vs runtime: dev‑smoke отмечен как dev‑only, runtime‑доки не требуют `dev/repo_tools/*`. **AC:** dev‑only упомянуты отдельно от runtime‑шагов.
...
- [x] Унифицировать корневую логику `tools.feature_ids.resolve_project_root`: убрать приоритет `CLAUDE_PLUGIN_ROOT` и делать детекцию только по workspace‑пути (`cwd` → `./aidd` → `.`, если `cwd` уже `aidd` — использовать его), чтобы записи не уходили в кеш плагина. **AC:** при установленном плагине в кеше и запуске из workspace все записи (`docs/.active_*`, PRD scaffold) остаются в `./aidd`; тесты `dev/tests/test_feature_ids_root.py` обновлены/проходят.
- [x] Убрать `--target` из runtime CLI (и из help/доков), оставить один источник истины: `cwd/aidd` как рабочий root. **AC:** `--target` не принимается нигде; при запуске из корня проекта всё пишется в `./aidd`; инструкции в `commands/*.md`/`dev/doc/workflow.md` не ссылаются на `--target`.
...
- [x] Выправить `hooks/format-and-test.sh`: убрать использование `CLAUDE_PLUGIN_ROOT` как кандидата project root; использовать `CLAUDE_SETTINGS_PATH`/workspace root и надёжный поиск `./aidd`. **AC:** форматтер/тесты пишут отчёты в workspace `aidd/reports/**`, не в кеш плагина; `dev/tests/test_post_hook_paths.py` обновлён и проходит.
- [x] Нормализовать относительные пути в gate‑скриптах (`tools/prd_review_gate.py`, `tools/plan_review_gate.py`, `tools/tasklist_check.py`): вычислять относительность от найденного project root, а не от `Path.cwd()`. **AC:** `--file-path` и report‑пути корректно обрабатываются при любом CWD; `dev/tests/test_gate_*` обновлены при необходимости.
- [x] Обновить документацию по root‑резолвингу и кешу плагина: явно зафиксировать, что рабочий root — это workspace, а `CLAUDE_PLUGIN_ROOT` используется только для ресурсов плагина. **AC:** `dev/doc/workflow.md` (и при необходимости README) описывают правило и не содержат двусмысленностей.
- [x] Зафиксировать в документации правила из офиц. доки Claude Code: плагин **копируется в кеш**, доступ к файлам **вне корня запрещён**, пути в `plugin.json` должны быть относительными `./`, а скрипты/хуки должны использовать `${CLAUDE_PLUGIN_ROOT}`; cwd хуков не гарантирован. **AC:** в `dev/doc/workflow.md` (и при необходимости README) есть явное описание и ссылка на `https://code.claude.com/docs/en/plugins-reference#plugin-caching-and-file-resolution`.

### EPIC L — Убрать `dev/` и вынести dev‑правила в `AGENTS.md`
- [ ] Перенести `dev/.github/workflows/ci.yml` в `.github/workflows/ci.yml` и обновить шаги на новые пути (`tests/repo_tools/ci-lint.sh`). **AC:** CI запускается из корня и не использует `dev/.github`.
- [ ] Перенести `dev/.markdownlint.yaml` и `dev/.pre-commit-config.yaml` в корень. **AC:** `tests/repo_tools/ci-lint.sh` использует корневой `.markdownlint.yaml`, pre‑commit работает без `dev/`.
- [ ] Перенести `dev/tests/**` → `tests/**` и обновить все ссылки/дискавери тестов. **AC:** `python3 -m unittest discover -s tests -t .` проходит, упоминаний `dev/tests` нет.
- [ ] Перенести `dev/repo_tools/**` → `tests/repo_tools/**` и обновить пути в скриптах/доках/хуках. **AC:** `tests/repo_tools/ci-lint.sh` и `tests/repo_tools/smoke-workflow.sh` запускаются из корня, `rg "dev/repo_tools"` пустой.
- [ ] Перенести `dev/doc/backlog.md` → `backlog.md` (корень) и обновить все ссылки. **AC:** `backlog.md` в корне, `rg "dev/doc/backlog.md"` пустой.
- [ ] Свернуть содержание `dev/doc/**` в корневой `AGENTS.md` (dev‑гайд, релизы, prompt‑versioning, reports‑format, migration, install‑decision, templates) и удалить `dev/doc/**`. **AC:** все dev‑правила доступны в `AGENTS.md`, `rg "dev/doc/"` пустой.
- [ ] Обновить все ссылки на новые пути (`README*`, `CHANGELOG.md`, `CONTRIBUTING.md`, `hooks/*`, `tests/repo_tools/*`, CI). **AC:** `rg "dev/"` не находит dev‑путей кроме возможных исторических примеров в `backlog.md`.
- [ ] Удалить каталог `dev/`. **AC:** каталога нет, тесты/линтеры/смоук работают из корня.
