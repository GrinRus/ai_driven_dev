...
- [x] `dev/doc/workflow.md`: создать пошаговый пример запуска `init-claude-workflow.sh` на демо‑проекте, включить скриншоты/структуру «до и после», обновить ссылки в README.
- [x] `dev/doc/customization.md`: описать настройку `.claude/settings.json`, `config/conventions.json`, хуков и команд, добавить примеры override для разных команд.
...
### Качество и процесс
- [x] `dev/.github/workflows/ci.yml`: настроить CI, запускающий shellcheck, линтеры и тесты скрипта на каждом PR.
- [x] `dev/repo_tools/ci-lint.sh`: собрать единый entrypoint для локального и CI‑прогонов (shellcheck, markdownlint, yamllint).
- [x] `dev/doc/release-notes.md`: зафиксировать процесс версионирования, формат релизов и чеклист публикации (теги, обновление README и демо).
...
- [x] `dev/doc/templates/git-hooks/`: собрать каталог примерных Git-хуков (commit-msg, pre-push, prepare-commit-msg) с инструкцией по установке.
...
- [x] `dev/repo_tools/smoke-workflow.sh`: добавить сценарий, который последовательно вызывает новые команды на демо-фиче, проверяет создание PRD/плана/тасклиста и что хук блокирует правки кода до готовности артефактов.
...
- [x] `dev/doc/workflow.md`: подробно описать новый цикл, роли агентов, формат вопросов/ответов с пользователем, обязанности перед каждым гейтом, включая новые проверки контрактов/миграций/тестов.
...
- [x] `dev/doc/customization.md`: добавить раздел «Как включить/отключить гейты и автотесты», описать `config/gates.json`, allowlist зависимостей и варианты кастомизации для проектов без Gradle и monorepo.
- [x] `dev/doc/workflow.md`: обновить сценарий, чтобы продемонстрировать работу новых команд и барьеров (создание API контракта, добавление миграции, автогенерацию тестов).

## Wave 4

### Playbook агентов и барьеров
- [x] `dev/doc/agents-playbook.md`: описать последовательность работы всех агентов (analyst → planner → validator → implementer → reviewer → api-designer → qa-author), список их команд и ожидаемые входы/выходы.
- [x] `dev/doc/agents-playbook.md`: документировать взаимодействие с барьерами (`gate-workflow`, `gate-tests`, `lint-deps`), условия срабатывания, способы обхода и troubleshooting.
...
- [x] `scripts/{branch_new.py,commit_msg.py,conventions_set.py}`: вернуть скрипты в репозиторий, синхронизировать их генерацию с `init-claude-workflow.sh` и описать использование в `dev/doc/workflow.md`; добавить unittest, который проверяет наличие CLI-утилит после установки. _(устарело, отменено в Wave 9)_
...
- [x] `dev/doc/customization.md` / `dev/doc/workflow.md`: обновить walkthrough и разделы настройки с учётом новой обзорной документации и сценария синхронизации README.

### CI и тестовый контур
- [x] `init-claude-workflow.sh`: устранить рассинхрон с текущим `dev/.github/workflows/ci.yml` (сейчас генерируется `dev/.github/workflows/gradle.yml`); выбрать единый pipeline и описать его в README и релиз-нотах.
- [x] `dev/tests/test_init_aidd.py`: дополнить проверками на наличие overview-документа, helper-скриптов и актуального CI-файла; добавить негативные сценарии (например, отсутствие dev/doc/intro) и очистку временных артефактов.

## Wave 7

### Claude Code feature presets
- [x] `docs/design/feature-presets.md`: описать архитектуру YAML-пресетов на все стадии фичи (PRD, дизайн, тасклист, реализация, релиз), определить обязательные поля (`workflow_step`, `context`, `output`) и схему интеграции с `dev/doc/workflow.md`.
...
- [x] `init-claude-workflow.sh`: добавить установку каталога пресетов, CLI-флаг `--preset <name>` и автоматическое заполнение шаблонов из `dev/doc/backlog.md`/`dev/doc/workflow.md`.
- [x] `.claude/commands/`: расширить описание `/feature-dev-aidd:idea-new`, `/feature-dev-aidd:plan-new`, `/feature-dev-aidd:tasks-new`, `/feature-dev-aidd:implement`, `/feature-dev-aidd:review` инструкциями по запуску соответствующих пресетов и обновить quick-reference таблицу.
- [x] `dev/repo_tools/smoke-workflow.sh`: включить сценарий end-to-end, который прогоняет пресеты по демо-фиче и проверяет генерацию PRD/ADR/tasklist.
- [x] `dev/doc/workflow.md`: обновить walkthrough с разделом «Работа с пресетами» и чеклистом интеграции для новых команд.

## Wave 8

### Чистка устаревших команд и документации
- [x] `.claude/commands/{feature-new.md,feature-adr.md,feature-tasks.md,docs-generate.md}`: удалить, синхронизировать quick-reference и init-скрипт под флоу `/feature-dev-aidd:idea-new → /feature-dev-aidd:plan-new → /feature-dev-aidd:tasks-new → /feature-dev-aidd:implement → /feature-dev-aidd:review`.
...
- [x] `dev/doc/workflow.md`: переписать walkthrough под новый цикл, добавить шаги `/feature-dev-aidd:idea-new → /feature-dev-aidd:plan-new → /feature-dev-aidd:tasks-new` и детальные проверки гейтов (workflow/API/DB/tests).

### Контроль использования CLI-утилит
- [x] `dev/doc/backlog.md` / `dev/tests/`: закрепить решение — Python-утилиты (`scripts/branch_new.py`, `scripts/commit_msg.py`, `scripts/conventions_set.py`) входят в репозиторий, командные файлы опираются на них; описать в README и убедиться, что init-скрипт и тесты поддерживают сценарий. _(устарело, отменено в Wave 9)_
...
- [x] `.claude/commands/feature-activate.md`: удалить или переместить в архив, зафиксировать решение в `README.md` и `dev/doc/workflow.md`.
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`: пересобрать разделы «Быстрый старт», «Слэш-команды», чеклист, убрав ссылки на исключённые команды и подчёркивая основной цикл `/feature-dev-aidd:idea-new → /feature-dev-aidd:plan-new → /feature-dev-aidd:tasks-new → /feature-dev-aidd:implement → /feature-dev-aidd:review`.
- [x] Тесты и хуки: убедиться, что `.claude/hooks/*` и `dev/tests/` не ссылаются на удалённые команды; скорректировать smoke-сценарии (`dev/repo_tools/smoke-workflow.sh`) и демо (`dev/doc/workflow.md`, `examples/apply-demo.sh`).

## Wave 10

### Чистка лишних слэш-команд
- [x] `.claude/commands/api-spec-new.md`: убрать из шаблона, удалить привязку к /api-spec-new во всех гидах (`README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`) и из чеклистов.
- [x] `.claude/commands/tests-generate.md`: удалить команду и ссылки; описать, как покрывать тестами в рамках `/feature-dev-aidd:implement` или прямого вызова агента.
...
- [x] `.claude/agents/api-designer.md`, `.claude/agents/qa-author.md`: перенести в раздел «advanced» или удалить после отказа от связанных команд; обновить `dev/doc/agents-playbook.md`.
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`: сфокусировать описание флоу на `/feature-dev-aidd:idea-new → /feature-dev-aidd:plan-new → /feature-dev-aidd:tasks-new → /feature-dev-aidd:implement → /feature-dev-aidd:review`, убрать секции про необязательные артефакты.
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `dev/doc/customization.md`: переписать сценарии, заменив вызовы `/api-spec-new` и `/tests-generate` на прямое использование агентов или ручные шаги.
...
- [x] `dev/repo_tools/smoke-workflow.sh`: упростить smoke-тест — проверять только базовый цикл и исключить требования к пресетам `feature-design/feature-release`.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`: добавить явное пояснение, как работает авто запуск тестов после записи и как временно отключить/расширить его без использования устаревшей команды.
...
- [x] Обновить документацию (`README.md`, `README.en.md`, `dev/doc/workflow.md`) — описать установку через `uv tool install claude-workflow-cli --from git+https://github.com/<org>/ai_driven_dev.git` и замену `curl | bash`; дополнить разделы о требованиях и обновлениях.
- [x] Добавить инструкцию для пользователей без `uv` (например, `pipx install git+...`) и секцию troubleshooting; проверить установку на чистой среде и зафиксировать сценарий в `dev/repo_tools/smoke-workflow.sh`.
...
- [ ] `dev/.github/workflows/release.yml`: триггер на теги `v*`; шаги — `uv build`/`python -m build`, публикация wheel+sdist в GitHub Release через `softprops/action-gh-release`, загрузка payload zip и manifest checksum; складывать артефакты и в CI.
...
- [ ] Документация (`README.md`, `README.en.md`, `dev/doc/workflow.md`): описать цепочку build → release → установка через `uv`/`pipx`, переменные/токены, troubleshooting для неудачных релизов.
- [ ] `CHANGELOG.md` / `dev/doc/release-notes.md`: добавить шаблон/секцию, которую release workflow подтягивает автоматически (последний раздел или Release Drafter); зафиксировать порядок обновления changelog перед тегом.
...
- [x] Написать регресcионный тест/скрипт, который собирает wheel и проверяет наличие `.claude/**` в архиве (например, `dev/tests/test_package_payload.py`).
...
- [x] После фикса обновить `README.md` / `dev/doc/workflow.md`: отметить, что установка через `uv` теперь не требует ручного вмешательства, и добавить инструкцию по обновлению (`uv tool upgrade`, `pipx upgrade`).
- [x] Прогнать smoke (`dev/repo_tools/smoke-workflow.sh`) и `uv tool install` на чистой директории, зафиксировать результат в `CHANGELOG.md`.
...
- [x] Обновить документацию (README, dev/doc/workflow.md, CHANGELOG) с описанием нового режима, инструкцией для пользователей и предупреждениями о бэкапах.
...
- [x] Переписать документацию (README, dev/doc/customization.md, dev/doc/workflow.md) с описанием разделения payload/локальных настроек и инструкцией по запуску bootstrap-скрипта.
...
- [x] `dev/doc/customization.md`, `dev/doc/workflow.md`, `README.md`: описать новый режим удалённого обновления payload, требования к токенам и офлайн-fallback.
...
- [x] `dev/.github/workflows/release.yml`: публиковать zip-архив payload рядом с wheel и прикладывать manifest checksum.
- [x] `CHANGELOG.md`, `dev/doc/release-notes.md`: задокументировать новые артефакты релиза и процесс отката до конкретной версии payload.
...
- [x] `dev/.github/workflows/ci.yml`, `dev/.github/workflows/gate-workflow.yml` (если потребуется) и `.claude/hooks/gate-workflow.sh`: встроить QA-гейт в общий пайплайн (локально + CI), предусмотреть опциональный запуск (`CLAUDE_SKIP_QA`, `--only qa`) и корректную композицию с другими gate-скриптами.
- [x] `dev/doc/qa-playbook.md` и `README.md`: зафиксировать процесс подготовки входных данных для QA-агента, примеры отчётов и интерпретацию статусов; обновить разделы usage/TL;DR с указанием, когда запускать гейт и какие артефакты прикладывать в PR.
- [x] `dev/tests/` и/или `dev/repo_tools/smoke-workflow.sh`: добавить сценарии, проверяющие вызов QA-агента, обработку блокирующих/некритичных дефектов и корректный вывод логов; использовать фикстуры/моки, чтобы детерминировать ответы агента.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `dev/doc/customization.md`: обновить флоу с шагом PRD Review, описать, когда вызывать агента, как интерпретировать вывод и какие поля нужно обновить в PRD/плане; привести пример output.
...
- [x] `dev/tests/`: внедрить автотесты `dev/tests/test_gate_prd_review.py` и `dev/tests/test_prd_review_agent.py`, эмулирующие разные статусы ревью, парсинг отчёта, конфигурацию гейта и поведение при блокерах/варнингах.
- [x] `dev/repo_tools/smoke-workflow.sh` и `README.md`: обновить энд-ту-энд сценарий, показывая этап PRD review (вызов команды, фиксация статуса, переход к планированию); зафиксировать влияние на developer experience и требования к артефактам.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`: обновить флоу — описать, какую информацию приносит Researcher, как читать отчёт и каким образом разработчик подтверждает, что рекомендации применены.
- [x] `dev/doc/workflow.md`, `dev/doc/qa-playbook.md`: добавить walkthrough с вызовом Researcher на демо-фиче, демонстрацией найденных участков кода и тем, как отчёт используется в QA/код-ревью.
- [x] `dev/tests/test_researcher_context.py`, `dev/tests/test_gate_researcher.py`: наметить автотесты, которые проверяют сбор контекста, корректность генерации отчёта и работу гейтов (включая edge-case: нет совпадений, конфликтующие рекомендации).
- [x] `dev/repo_tools/smoke-workflow.sh`, `dev/repo_tools/ci-lint.sh`: включить Researcher в smoke-сценарий (generate context → вызвать агента → проверить наличие отчёта) и покрыть линтером структуру новых шаблонов.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`: расширить описание роли analyst с пошаговым Q&A-циклом, критерия READY и форматом фиксации ответов.
- [x] `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`: включить пример диалога (вопрос → ответ → уточнение) и подчеркнуть, что без закрытия вопросов фича остаётся BLOCKED.
...
### Контроль качества
- [x] `dev/repo_tools/smoke-workflow.sh`, `dev/tests/test_analyst_dialog.py`: внедрить проверку, что первый вывод агента содержит «Вопрос 1», PRD не получает статус READY до явного набора ответов и что формат ответов соблюдён.
...
- [x] `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `README.md`, `README.en.md`: обновить схемы и walkthrough, подчёркивая, что tasklist теперь хранится в `docs/tasklist/<ticket>.md`.
- [x] `dev/tests/test_gate_workflow.py`, `dev/tests/test_qa_agent.py`, `dev/tests/test_gate_researcher.py`, `dev/repo_tools/smoke-workflow.sh`: адаптировать тесты и smoke-сценарий под новую структуру tasklist, покрыть миграцию и параллельные slug'и.
- [x] `CHANGELOG.md`, `dev/doc/release-notes.md`, `dev/doc/feature-cookbook.md`: зафиксировать переход на feature-ориентированный tasklist и обновить примеры использования.
...
- [x] `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `dev/tests/test_gate_tests_hook.py`, `dev/repo_tools/smoke-workflow.sh`: зафиксировать новый порядок действий, обновить чеклисты и покрыть сценарий запуска тестов по запросу reviewer/ручному override.
...
- [x] `dev/tests/test_gate_workflow.py`, `dev/tests/test_qa_agent.py`, `dev/repo_tools/smoke-workflow.sh`: расширить тесты, проверяющие, что новый прогресс-чек применяется, и зафиксировать сценарии с несколькими итерациями и отсутствием обновления чекбоксов.

### Документация и обучение команды
- [x] `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `dev/doc/qa-playbook.md`: переписать walkthrough, подчёркивая итеративное отмечание прогресса и необходимость ссылаться на обновлённые чекбоксы в ответах агентов.
...
- [x] `config/gates.json`, `.claude/hooks/format-and-test.sh`, `.claude/hooks/gate-workflow.sh`, `.claude/hooks/gate-tests.sh`, `dev/repo_tools/smoke-workflow.sh`: обновить проверки и отчёты на использование `{ticket}` вместо `{slug}`, прокидывая slug только как дополнительную подсказку в логах и отчётах.
...
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `dev/doc/qa-playbook.md`: переписать walkthrough и примеры команд под модель TICKET-first, описать роль slug-хинта как пользовательского ориентира.
- [x] `CHANGELOG.md`, `dev/doc/release-notes.md`, `dev/doc/feature-cookbook.md`, `dev/tests/test_gate_workflow.py`, `dev/tests/test_qa_agent.py`: подготовить миграцию Wave 26 (slug → ticket-first), обновить релизные заметки и покрыть сценарии тестами.
...
- [x] `init-claude-workflow.sh`, `dev/repo_tools/smoke-workflow.sh`: синхронизировать bash-обёртку с новой структурой, покрыть пустую/существующую директорию, гарантировать идентичную логику с CLI и новой раскладкой `aidd/`. _Сделано: init + smoke работают с поддиректорией, тесты зелёные._
...
- [x] `.claude/commands/researcher.md`, `docs/research/template.md`: добавить блок «Отсутствие паттернов» и обязательную секцию `## Паттерны/анти-паттерны`, описать, как оформлять вывод для пустого проекта.
...
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `README.md`: описать обновлённый порядок `/feature-dev-aidd:idea-new → claude-workflow research → analyst`, правила для пустых репозиториев и требования к паттернам.
- [x] `dev/tests/test_gate_researcher.py`, `dev/tests/test_gate_workflow.py`: добавить сценарии для «project new» (нулевые совпадения) и для кейса с найденными паттернами, проверять, что гейт отрабатывает предупреждение.
- [x] `dev/repo_tools/smoke-workflow.sh`, `examples/gradle-demo/`: показать новую последовательность и пример отчёта Researcher с перечислением найденных паттернов.
...
- [x] `dev/tests/test_gate_prd_review.py`, `dev/tests/test_analyst_guard.py`, `dev/repo_tools/smoke-workflow.sh`: покрыть сценарий автосозданного PRD (файл есть, но статус draft) и убедиться, что гейты выключают ошибку «нет PRD» и переходят к содержательной проверке.

### Документация и обучение
- [x] `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, `dev/doc/agents-playbook.md`: описать, что `/feature-dev-aidd:idea-new` сразу создаёт PRD-шаблон, поэтому гейты больше не просят перезапуск; выделить обязанности агента (заполнить `## Диалог analyst`, снять статус draft).
- [x] `dev/doc/release-notes.md`, `CHANGELOG.md`: зафиксировать Wave 30 как обновление UX аналитики, перечислить шаги автосоздания PRD и обновлённые сообщения гейтов.
...
- [x] `tools/check_payload_sync.py` + CI/pre-commit: написать проверку, которая сравнивает контрольные суммы payload-контента и корневых «runtime snapshot» файлов. Если обнаружены расхождения без фиксации `sync --direction=from-root`, тест/CI должен падать. Добавить запуск в `dev/.github/workflows/ci.yml` и как pre-commit hook.
- [x] Документация и процессы: в `dev/doc/customization.md`, `CONTRIBUTING.md`, `dev/doc/workflow.md` описать правило «редактируем только payload → синхронизуем скриптом». В release checklist добавить обязательный шаг `scripts/sync-payload.sh --direction=from-root && pytest dev/tests/test_init_hook_paths.py` перед `uv publish`. Упомянуть, что для локальной проверки нужно использовать `scripts/bootstrap-local.sh --payload src/.../payload`, а не трогать `.claude` вручную.
- [x] Тесты: расширить `dev/tests/test_init_hook_paths.py` и/или создать `dev/tests/test_payload_sync.py`, который проходит по списку критичных файлов (хуки, `init-claude-workflow.sh`, шаблоны docs) и проверяет, что payload и root синхронизированы. Тест должен использовать общий helper для расчёта хэшей и работать от `src/claude_workflow_cli/data/payload`.
- [x] CI/tooling: обновить `dev/repo_tools/ci-lint.sh` и `Makefile` (если появится) так, чтобы новые проверки запускались локально командой `scripts/sync-payload.sh --direction=from-root && python tools/check_payload_sync.py`. Зафиксировать рекомендацию в `dev/doc/backlog.md` для последующих волн.

## Wave 32

- [x] `dev/doc/prompt-playbook.md` (новый), `README.md`, `dev/doc/agents-playbook.md`: зафиксировать обязательные секции для агентов/команд (Контекст, Входы, Автоматизация, Формат ответа, Fail-fast), описать требования к строке `Checkbox updated`, ссылкам на `docs/prd|plan|tasklist`, правилам эскалации блокеров и матрицу «роль → артефакты/хуки`.
- [x] `dev/doc/templates/prompts/prompt-agent.md`, `dev/doc/templates/prompts/prompt-command.md`, `claude-presets/advanced/prompt-governance.yaml` (новый): добавить шаблоны фронт-маттера (`name/description/tools/inputs/outputs/hooks`) и готовые заголовки, а также preset/скрипт развёртывания (`scripts/scaffold_prompt.py` или Make target) с примерами использования в CLI.
...
- [x] `dev/repo_tools/lint-prompts.py`, `dev/repo_tools/ci-lint.sh`, `dev/.github/workflows/ci.yml`, `dev/tests/test_prompt_lint.py`: добавить линтер промптов, который валидирует фронт-маттер, наличие обязательных секций, консистентность между парами агент↔команда (например, implementer/implement), и включить его в CI/pre-commit.
...
- [x] `dev/doc/prompt-playbook.md`, `dev/doc/prompt-versioning.md` (новый): описать политику двух языков, правила синхронизации (когда менять обе версии, как обозначать отличия), формулу версионирования (`major.minor.patch`, где major — изменение структуры, minor — правки текста, patch — правки примечаний), и процедуру ревью (обновление changelog/prompts.json).
- [x] `dev/doc/release-notes.md`, `CHANGELOG.md`: задокументировать запуск двуязычных промптов и версионирования, включая инструкцию «как откатиться к предыдущей версии промпта» (git tag/manifest), и добавить пример записи в release checklist (`dev/repo_tools/prompt-version bump --lang ru,en`).

### Тесты и проверка качества
- [x] `dev/repo_tools/smoke-workflow.sh`, `dev/tests/test_gate_workflow.py`: добавить smoke-тест на gate, который редактирует только один язык и убеждается, что gate блокирует merge; предусмотреть позитивный сценарий, где обе локали обновлены и gate пропускает изменения.
- [x] `dev/repo_tools/ci-lint.sh`: включить новые тесты и эмулировать минимум один прогон `dev/repo_tools/prompt-version bump --lang ru,en` для проверки версионирования.

- [x] `dev/tests/test_prompt_versioning.py`: добавить тесты `dev/repo_tools/prompt-version` для команд и ситуаций с частичным bump (например, только ru, только en, skip), проверить обновление `source_version`.
- [x] `dev/repo_tools/smoke-workflow.sh`, `dev/tests/test_gate_workflow.py`: расширить сценарии gate для команд и `Lang-Parity: skip`, включить проверку, что после удаления skip гейт снова требует синхронное обновление.
...
- [ ] `tools/run_cli.py` (новый), `tools/set_active_feature.py`, `.claude/hooks/gate-workflow.sh`, `.claude/hooks/format-and-test.sh`, `dev/repo_tools/smoke-workflow.sh`, `scripts/qa-agent.py`: внедрить helper `run_cli(command: list[str])`, который ищет бинарь `claude-workflow` в PATH (поддержка uv/pipx шима), умеет читать `CLAUDE_WORKFLOW_BIN`/`CLAUDE_WORKFLOW_PYTHON`, а при отсутствии печатает инструкцию «установите CLI командой …»; все скрипты должны использовать helper вместо прямого запуска python‑модулей, чтобы пользователю хватало шагов установки из README.
...
- [ ] `dev/tests/test_cli_entrypoint.py` (новый), `dev/repo_tools/smoke-workflow.sh`, `dev/repo_tools/ci-lint.sh`: добавить проверки, эмулирующие чистую систему (без dev-src, только бинарь `claude-workflow`), и убедиться, что helper корректно находит CLI; при отсутствии бинаря тесты должны давать понятное сообщение и ссылку на команды установки.
  ...
  - `dev/repo_tools/smoke-workflow.sh` перед вызовом helper сбрасывает override‑пути, временно скрывает `src` (например, запускается в подпапке без dev-src) и использует `run_cli` для всех команд, чтобы подтвердить zero-touch E2E;
  - `dev/repo_tools/ci-lint.sh` явно добавляет `python3 -m unittest discover -s dev/tests -t dev -p "test_cli_entrypoint.py"` (помимо общего `unittest` прогона), чтобы регрессии shim падали до merge.
## Wave 34
### Agent-first промпты и гайды
- [x] `dev/doc/prompt-playbook.md`, `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `dev/doc/backlog.md`: переписать правила с упором на «agent-first» модель (агент сам читает файлы и запускает скрипты, вопросы пользователю — крайняя мера), убрать упоминания человеческих ресурсных ограничений и добавить примеры автоматических источников данных.
- [x] `README.md`, `README.en.md`, `dev/doc/release-notes.md`, `CHANGELOG.md`: задокументировать переход на «agent-first» (новый раздел в README, запись в релизных заметках, чеклист миграции существующих проектов).
...
- [x] `dev/doc/templates/prompts/prompt-agent.md`, `dev/doc/templates/prompts/prompt-command.md`, `src/claude_workflow_cli/data/payload/templates/{prompt-agent.md,prompt-command.md}`: обновить подсказки — вместо «задавайте вопросы» прописать обязательные блоки по чтению артефактов, запуску утилит и фиксации артефактов в ответе.
...
- [x] `dev/tests/test_cli_sync.py`, `src/claude_workflow_cli/data/payload/tests/test_payload_sync.py`: расширить тесты синхронизации на новые файлы (обновлённые шаблоны и промпты), чтобы не потерять «agent-first» инструкции в дистрибутиве.
- [x] `dev/repo_tools/ci-lint.sh`, `dev/.github/workflows/ci.yml`: подключить новые проверки (например, `rg 'Answer [0-9]'` → провал, если в промпте остались упоминания ручного Q&A), прогонять их в CI.

### Коммуникация и миграция
- [x] `dev/doc/feature-cookbook.md`, `dev/doc/customization.md`, `dev/doc/workflow.md`: добавить раздел «Как мигрировать существующие проекты на agent-first» (шаги обновления шаблонов, прогон sync+tests, чеклист по обновлению `.claude/agents`).
...
- [x] `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `dev/doc/qa-playbook.md`, `README.md`, `README.en.md`: включить обязательный шаг `/feature-dev-aidd:qa` (после `/feature-dev-aidd:review`) в walkthrough/quick-start, перечислить входы (diff, tasklist QA, логи гейтов), параметры гейта (`CLAUDE_SKIP_QA`, `--only qa`, dry-run), формат отчёта и прогресс-маркеры.
...
- [x] `scripts/qa-agent.py`: синхронизировать опции/формат JSON с CLI (`--ticket/--slug-hint/--branch/--report/--block-on/--warn-on/--dry-run`), описать режимы `--gate` и интерактив в `dev/doc/qa-playbook.md`.

### Тесты и payload
- [x] `dev/tests/test_gate_qa.py`, `dev/repo_tools/smoke-workflow.sh`: unit + полный дымовой сценарий: READY/WARN/BLOCKED, отсутствие отчёта (должно падать), `--only qa`, dry-run (не падает на блокерах), обновление tasklist/`Checkbox updated`; включить в CI.
...
- [x] Документация: README/dev/doc/workflow.md/agents-playbook — кратко зафиксировать автозапуск аналитика, логи повторного research и требование `analyst-check` после READY. _(закрыто за ненадобностью)_

## Wave 37

### Удаление внутреннего backlog из дистрибутива
- [x] `dev/doc/backlog.md`: оставить файл только для разработки (dev-only), исключив его из устанавливаемого payload; при необходимости перенести в `docs/dev-backlog.md` или пометить как ignore для sync.
- [x] `src/claude_workflow_cli/data/payload/manifest.json`, `tools/check_payload_sync.py`, `dev/tests/test_payload_manifest.py`, `dev/tests/test_package_payload.py`: удалить запись о `dev/doc/backlog.md`, обновить проверку состава payload и убедиться, что сборка wheel/zip не тянет файл.
- [x] `scripts/sync-payload.sh`, `src/claude_workflow_cli/data/payload/tests/test_payload_sync.py`: скорректировать списки путей, чтобы sync не требовал `dev/doc/backlog.md`; добавить тест, который гарантирует отсутствие dev-only файлов в payload.
- [x] Документация (`dev/doc/prompt-playbook.md`, `dev/doc/release-notes.md`, `CHANGELOG.md`): отметить, что backlog не поставляется конечным пользователям, добавить запись в release checklist и guidance по ведению roadmap в репо без включения в payload.
- [x] Проверка и smoke: прогнать `dev/repo_tools/ci-lint.sh`, `tools/check_payload_sync.py`, smoke-сценарии сборки дистрибутива, удостовериться в корректной установке через `uv/pipx` без `dev/doc/backlog.md` в артефактах.
...
- [x] `dev/tests/test_researcher_context.py`, `dev/tests/test_gate_researcher.py`, `dev/repo_tools/smoke-workflow.sh`: покрыть глубокий режим (поиск функций/классов, reuse-кандидаты, ссылки на тесты), негативные сценарии без совпадений и проверку, что отчёт содержит обязательные секции; убедиться, что payload-копии проходят sync/manifest проверки и что call graph делегирован агенту.
- [x] `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, `README.md`, `README.en.md`: описать новый формат работы Researcher (deep-code + reuse), обязательные поля отчёта, примеры интерпретации и как применять рекомендации в план/тасклист; добавить guidance по запуску с фильтрами директорий/языков и явным шагом построения графа на стороне Claude Code.
...
- [x] `dev/doc/feature-cookbook.md`, `dev/doc/workflow.md`, `README.md`, `README.en.md`: описать рекомендуемый вариант реализации планов — базовый сервисный слой + адаптеры к внешним системам, reuse существующих утилит/клиентов, явное ограничение областей (domain/application/infra), ссылки на паттерны и тестовые стратегии.
- [x] `dev/tests/test_planner_agent.py`, `dev/repo_tools/smoke-workflow.sh`: добавить проверки, что план содержит секцию про архитектурные решения и KISS/YAGNI/DRY, перечисляет reuse-точки и не предлагает избыточных компонентов; обновить smoke-сценарий с примером плана, использующего рекомендованный паттерн (service-layer + adapters).
...
- [x] Researcher — проверки: дополнить `dev/tests/test_researcher_context.py`, добавить `test_gate_researcher.py`, обновить smoke для deep-скана и статуса; убедиться, что sync/manifest учитывают новые артефакты.
...
- [x] Planner — тесты/доки: добавить `dev/tests/test_planner_agent.py`, обновить smoke, задокументировать рекомендованную структуру (domain/app/infra) и применение паттернов в `dev/doc/workflow.md`/cookbook/README.
...
- [x] Тесты: `dev/tests/test_researcher_context.py` + фикстур на call graph (kt/java), `dev/tests/test_researcher_call_graph.py` — проверка рёбер, fallback без tree-sitter, негативные кейсы; e2e `dev/tests/test_researcher_call_graph_e2e.py` на реальных `.java/.kt`; обновлён smoke, чтобы проверять наличие `call_graph` в JSON.
...
- [x] Документация: `dev/doc/agents-playbook.md`, `dev/doc/workflow.md`, `README.md`, `README.en.md`, `dev/doc/feature-cookbook.md` — описать опцию call graph (когда использовать, параметры CLI, ограничения на языки), примеры вывода и влияние на гейты/планирование.
...
- [x] Тесты: `dev/tests/test_tasks_derive.py` — генерация чекбоксов из QA/Review/Research отчётов (ok/warn/block), idempotent append, dry-run; e2e в `dev/repo_tools/smoke-workflow.sh` (готовый отчёт → handoff → implement).
...
- [x] Документация: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/agents-playbook.md`, `docs/tasklist/template.md` — добавить шаг handoff после отчётов, пример вызова `tasks-derive`, формат маппинга finding → чекбокс и роль implementer в закрытии этих пунктов.
...
- [x] Тесты/доки: дополнить `dev/tests/test_gate_qa.py`, `dev/tests/test_qa_agent.py`, `dev/tests/test_tasks_derive.py`, `dev/repo_tools/smoke-workflow.sh` сценариями «QA запускает тесты, пишет логи, handoff обновляет tasklist, progress проходит/проваливается без `[x]`»; обновить `dev/doc/qa-playbook.md`/`README`/`dev/doc/workflow.md` с примерами команд, формата отчёта и минимальным чеклистом логов.
...
- [x] CLI `analyst` (обёртка для агента, auto-mode по дефолту) _(закрыто за ненадобностью)_
  ...
  - [x] Тесты/смоук: добавить сценарий `claude-workflow analyst --ticket demo --auto` (использует demo payload) в `dev/repo_tools/smoke-workflow.sh` и/или unit на парсер; убедиться, что `invalid choice: 'analyst'` больше не воспроизводится.
...
- [ ] `.claude/commands/idea-new.md`, `prompts/en/commands/idea-new.md`, `README.md`/`dev/doc/workflow.md`: явно требовать короткий ticket/slug (1 строка, до 80 символов, латиница/цифры/`-`), описать auto-sanitize и что длинные описания должны идти в PRD/idea, а не в идентификатор.
...
### Тесты и смоук
- [ ] `dev/tests/test_feature_ids.py`, `dev/tests/test_format_and_test_hook.py` (или новый): кейсы с длинным/многострочным тикетом → safe slug + хэш, отсутствие OSError, корректный путь к `reports/reviewer/...`.
- [ ] `dev/repo_tools/smoke-workflow.sh`: сценарий с намеренно длинным описанием тикета → auto-sanitize, успешный запуск хуков/тестов; убедиться, что логи содержат подсказку и что отчёты создаются по safe-имени.
...
- [ ] `.claude/commands/researcher.md`, `prompts/en/commands/researcher.md`, `/feature-dev-aidd:idea-new`: синхронизировать дефолтные параметры (JVM → call graph+deep-code, non-JVM → fast scan), описать эскалацию дополнительных `--paths/--keywords` и повторный запуск с graph-focus.
- [ ] Документация: `dev/doc/agents-playbook.md`, `dev/doc/feature-cookbook.md`, `dev/doc/workflow.md` — добавить таблицу «когда запускать graph vs обычный ресерч», примеры WARN/INSTALL_HINT, troubleshooting для пустого контекста.
- [ ] Тесты/смоук: кейсы `claude-workflow research --auto` в JVM-репо (строится граф, сохраняется полный JSON), в non-JVM (граф не строится, WARN), и «0 matches → baseline note». Обновить `dev/tests/test_researcher_context.py`, `dev/tests/test_gate_researcher.py`, `dev/repo_tools/smoke-workflow.sh`.
...
- [x] Спроектировать плагинные hook events (PreToolUse/PostToolUse/UserPromptSubmit/Stop/SubagentStop) через `hooks.json`: workflow-gate (PRD/plan/tasklist), dev/tests/format, anti-vibe prompt-gate, QA, post-write `tasks-derive`/`progress`, учесть `config/gates.json` и dry-run.
...
- [x] Обновить unit/smoke проверки хуков (`dev/tests/test_gate_workflow.py`, `dev/tests/test_gate_tests_hook.py`, `dev/tests/test_gate_qa.py`, `dev/repo_tools/smoke-workflow.sh`) под плагинные пути и сценарии PostToolUse/PostWrite.
...
- [x] Обновить гайды (`aidd/workflow.md`, `dev/doc/prompt-playbook.md`, `dev/doc/agents-playbook.md`, текущая статья) с примерами `$1/$ARGUMENTS`, `argument-hint`, `@docs/...`, схемой hook events и установкой плагина в поддиректорию `aidd/`.
- [x] Добавить ссылки на `dev/doc/workflow.md` и `config/conventions.md` в `aidd/CLAUDE.md`, вписав в существующий текст без перетирания содержимого.
...
- [x] После обновления хуков/смоука прогнать `python -m pytest dev/tests/test_gate_workflow.py dev/tests/test_gate_tests_hook.py dev/tests/test_gate_qa.py` и `dev/repo_tools/smoke-workflow.sh`; зафиксировать результаты.
...
- [x] Документация (`README*`, `dev/doc/workflow.md`, `aidd/CLAUDE.md`, `dev/doc/agents-playbook.md`): явное разделение project `.claude/` vs plugin `.claude-plugin/`, примеры подключения marketplace из подпапки `aidd/`, новые пути хуков.
...
- [x] Обновить документацию (README/README.en/dev/doc/workflow.md/CONTRIBUTING.md) и release checklist с правилами: что считается runtime-артефактом, что dev-only, как проводить аудит перед тэгом и где оставлять решение по удалённым файлам.
- [x] Оставить единый `aidd/init-claude-workflow.sh` в payload, убрать корневой дубликат и перевести smoke/dev/tests/docs на путь из payload.

## Wave 48

_Статус: новый, приоритет 3. Цель — аудит и упорядочивание корня репозитория (dev-only артефакты, дубли документации)._

- [x] Провести инвентаризацию корневых каталогов (`dev/doc/`, `docs/`, `scripts/`, `tools/`, `CLAUDE.md`, `dev/doc/workflow.md`), составить таблицу dev-only vs дистрибутив и отметить, что реально нужно пользователю.
- [x] Убрать/переместить dev-only материалы (design/feature-presets, backlog и пр.) в `dev/doc/`; обновить ссылки из README/CONTRIBUTING, чтобы не было битых путей после чистки.
...
- [x] Repo-only tooling: вынести `sync-payload.sh`, `lint-prompts.py`, `prompt-version`, `check_payload_sync.py`, `payload_audit.py` из payload, обновить docs/dev/tests/manifest и дефолтный sync.
- [x] Repo-only tooling: убрать `aidd/dev/repo_tools/ci-lint.sh` из payload, перенести его проверки в корневой `dev/repo_tools/ci-lint.sh`, обновить упоминания в доках/промптах, manifest и проверки payload.
...
### Тесты и смоук
- [x] `dev/tests/test_gate_tests_hook.py` (и новые тесты): проверить наличие `PostToolUse` команд и корректные пути (`gate-tests.sh`, `format-and-test.sh`, `lint-deps.sh`), сценарий idea-new без research → analyst требует research → после reports/research READY гейт пропускает план/код.
- [x] `dev/repo_tools/smoke-workflow.sh`: добавить кейс запуска из корня без `docs/` (используется `aidd/docs`), убедиться, что хуки не падают по отсутствующим путям и что `format-and-test`/`lint-deps` срабатывают после записи.
...
- [x] Обновить `dev/repo_tools/smoke-workflow.sh`: сценарий `/feature-dev-aidd:idea-new` с авто-analyst+auto-research, подтверждение корневого запуска без `docs/`, готовность после ответов/исправлений.
...
- [x] Синхронизация payload: пересобрать manifest после правок скриптов/команд, прогнать проверки состава payload (`tools/check_payload_sync.py`/`dev/tests/test_package_payload.py`) и убедиться, что новые пути зафиксированы.
...
- [x] Тесты CLI/init: новые проверки «init без `aidd/` → ошибка», «init в workspace → создаёт `aidd/`», покрыть `_resolve_roots/_require_workflow_root` и подсказки по таргету; обновить `dev/tests/test_init_*`, `dev/tests/test_cli_payload.py`, `dev/tests/test_bootstrap_e2e.py`.
...
- [x] Инструменты/скрипты: `aidd/tools/set_active_feature.py`, `aidd/dev/repo_tools/smoke-workflow.sh`, `aidd/tools/run_cli.py` — дефолт `--target aidd`, жёсткая проверка, WARN/exit при попытке писать в корень; подправить `aidd/config/gates.json` (feature_ticket_source/slug_hint_source → `aidd/docs/.active_*` если запуск из корня).
...
### Дока и DX
- [x] Обновить `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/design/install-subdir.md`, `dev/doc/adr/install-subdir-decision.md`, `aidd/dev/doc/agents-playbook.md`: убрать советы про произвольный `--target`, показать единственную установку `claude-workflow init --target .` → `./aidd`, troubleshooting «CLI не найден/не видит .claude» с ссылкой на фиксированный путь.
- [x] Добавить миграционную заметку: как перевести старые установки с корневым `.claude`/`docs` в `aidd/` (удалить dev-снапшоты, запустить init, перенести активные `.active_*` и артефакты), прописать в release notes и в dev/doc/backlog (готово после выполнения).

### Тесты/смоук/CI
- [x] Обновить `aidd/dev/repo_tools/smoke-workflow.sh`: убрать сценарий «корень без docs → fallback aidd», добавить проверку на ошибку при неправильном таргете и успешный happy-path только через `aidd/`; синхронизировать вызовы `set_active_feature.py` с новым `--target`.
- [x] Покрыть хуки: `dev/tests/test_gate_workflow.py`, `dev/tests/test_gate_tests_hook.py`, `dev/tests/test_gate_qa.py` — убедиться, что пути резолвятся из `aidd/`, нет попыток читать `./docs`, PostToolUse/PreToolUse команды не используют workspace env.
- [x] Обновить payload/manifest после правок; добавить regression-тест на `resolve_project_root` и отсутствие fallback в `dev/tests/test_resources.py` (или новый тест).
...
- [ ] Команда `/feature-dev-aidd:researcher` (RU/EN): описать авто-сбор call graph (без ручного `--call-graph`), опцию `--graph-engine none`, и применение графа для поиска точек встройки/зависимостей.
...
- [ ] `scripts/qa-agent.py`: перед импортами добавить fallback в `sys.path` для `.claude/hooks/_vendor` (и `CLAUDE_WORKFLOW_DEV_SRC`), вычислять путь от `${CLAUDE_PLUGIN_ROOT}`/`$PWD`, чтобы агент работал даже при «чистом» системном Python.
...
- [ ] Payload/manifest: после изменений в QA-агенте и путях обновить payload манифест и синк-тесты (`tools/check_payload_sync.py`/`dev/tests/test_package_payload.py`), убедиться, что `_vendor` остаётся в дистрибутиве.
...
- [x] `README.md` / `README.en.md`: обновить TL;DR и быстрый старт под cli `claude-workflow` (init/preset/sync/upgrade/smoke), добавить компактную справку по командам (analyst-check/research/reviewer-dev/tests/tasks-derive/qa/progress), явно подчеркнуть layout `aidd/` и синхронизировать Last sync.
...
- [x] Обновить описания в `src/claude_workflow_cli/data/payload/aidd/CLAUDE.md` или `.../dev/doc/customization.md`: что такое Working Set, где лежат отчёты, как менять лимиты/отключать guard'ы.
...
- [x] `dev/tests/test_context_gc.py`: добавить тесты на token‑mode (warn/block), включая парсинг JSONL из transcript и fallback на bytes.
...
- [x] Расширить `dev/repo_tools/lint-prompts.py`: проверка дубликатов ключей фронт‑маттера, запрет неизвестных статусов, поиск `&lt;ticket&gt;`, контроль формата `Checkbox updated` и совпадения `allowed-tools` с инструментами агента.
- [x] Обновить тесты `dev/tests/test_prompt_lint.py` и/или `dev/tests/test_prompt_versioning.py` под новые правила; добавить фикстуры на рассинхрон инструментов и дубликаты ключей.
- [x] После правок промптов: увеличить `prompt_version` в затронутых файлах, обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/release-notes.md`, `CHANGELOG.md`, `src/claude_workflow_cli/data/payload/manifest.json` и прогнать `python3 tools/check_payload_sync.py`.
...
- [x] Обновить тесты/смоук: `dev/tests/test_gate_workflow.py`, `dev/tests/test_gate_tests_hook.py`, `dev/tests/test_gate_qa.py`, `src/claude_workflow_cli/data/payload/aidd/dev/repo_tools/smoke-workflow.sh` под новую логику событий/дебаунса.
- [x] Документация: `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/agents-playbook.md` — обновить описание точки запуска гейтов и новых guard'ов.
...
- [x] Синхронизировать порядок стадий и ссылки на статусы во всех документах: `README.md`, `README.en.md`, `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `AGENTS.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/agents-playbook.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/feature-cookbook.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md` (ссылки на `sdlc-flow.md` и `status-machine.md`).
...
- [x] Стандартизировать вопросы пользователю (Blocker|Clarification → зачем → варианты → default) в `analyst`, `validator` (и при необходимости `planner`); обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md`.
...
- [x] Traceability: связать PRD acceptance criteria с QA‑проверками в `src/claude_workflow_cli/data/payload/aidd/agents/qa.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/qa-playbook.md`, при необходимости — в `src/claude_workflow_cli/data/payload/aidd/docs/prd/template.md`.

### Governance и проверки
- [x] Расширить `dev/repo_tools/lint-prompts.py`: контроль ссылок на `status-machine.md`/`sdlc-flow.md`, проверка шаблона вопросов в ключевых агентах; обновить `dev/tests/test_prompt_lint.py`.
- [x] Обновить smoke/regression под новый flow: `src/claude_workflow_cli/data/payload/aidd/dev/repo_tools/smoke-workflow.sh`, `dev/tests/test_gate_workflow.py`, `dev/tests/test_prompt_versioning.py`.
- [x] После правок: bump `prompt_version` (RU/EN), обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/release-notes.md`, `CHANGELOG.md`, `src/claude_workflow_cli/data/payload/manifest.json`, прогнать `python3 tools/check_payload_sync.py`, синхронизировать root через `scripts/sync-payload.sh --direction=to-root`.
...
- [x] Документация процесса: синхронизировать `dev/doc/workflow.md`, `dev/doc/feature-cookbook.md`, `dev/doc/prompt-playbook.md` (и payload‑версии) с новым порядком idea → research → plan и раздельными гейтами.
- [x] Smoke/тесты: обновить `src/claude_workflow_cli/data/payload/aidd/dev/repo_tools/smoke-workflow.sh` и/или `dev/tests/test_gate_workflow.py` под разделённые этапы и гейты.
- [x] Версионирование промптов и релизные заметки: bump `prompt_version` в изменённых командах/агентах (RU/EN), обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/release-notes.md`, `CHANGELOG.md`, `src/claude_workflow_cli/data/payload/manifest.json`, затем `scripts/sync-payload.sh --direction=to-root` и `python3 tools/check_payload_sync.py`.
...
- [x] Зафиксировать canonical/sync политику в `dev/doc/customization.md` (или новый `dev/doc/payload-policy.md`): что является источником правды и какие команды sync использовать.
- [x] Проверить `tools/check_payload_sync.py`/`tools/payload_audit_rules.json` на новые артефакты (`aidd/docs/anchors/`, `dev/doc/reports-format.md`, `aidd/docs/index/`) и обновить при необходимости.
- [x] Добавить раздел “Performance KPIs” в `dev/doc/prompt-playbook.md` (минимум: Stop/checkbox, формат‑тесты/checkbox, частота чтения reports, средний stdout логов).
...
- [x] Backfill anchors: `dev/repo_tools/upgrade_aidd_docs.py` — пройти по `aidd/docs/{prd,plan,tasklist,research}/**` и добавить отсутствующие `## AIDD:*` секции без перезаписи содержимого.
- [x] Тесты для апгрейда: `dev/tests/test_upgrade_aidd_docs.py` (минимум 1–2 фикстуры).
- [x] Контракт отчётов (MVP): `dev/doc/reports-format.md` (+ payload копия) — naming `reports/<type>/<ticket>-<kind>.pack.yaml`, правило pack‑first, budgets для `reports/research/*-context.pack.yaml`, deterministic output (byte‑identical), whitelist/blacklist полей.
- [x] Инвентаризация отчётов: `dev/repo_tools/report_stats.py` → таблица top‑3 hotspots с `chars/lines/keys` в `dev/doc/reports-format.md`.
...
- [x] Тесты: `dev/tests/test_reports_pack.py` (golden + детерминизм), `dev/tests/test_tasks_derive.py` (pack‑first), обновить `src/claude_workflow_cli/data/payload/manifest.json` и прогнать `python3 tools/check_payload_sync.py`.
...
- [x] Линт: расширить `dev/repo_tools/lint-prompts.py` на проверку anchors в шаблонах и наличие stage‑anchors; обновить `dev/tests/test_prompt_lint.py`.
- [x] Документация: обновить `dev/doc/{agents-playbook,prompt-playbook}.md`, `README.md`, `README.en.md`, `src/claude_workflow_cli/data/payload/aidd/workflow.md` под anchors‑first + working set.
- [x] DoD: контракт `dev/doc/reports-format.md` зафиксирован, pack создаётся для research‑context и используется в `tasks-derive`, golden‑тесты детерминизма проходят, root→payload sync выполнен.
...
- [x] Зафиксировать канонический путь `aidd/reports/context/latest_working_set.md` и описание в `dev/doc/customization.md`.
...
- [x] Регрессии: `dev/tests/test_format_and_test.py`, `dev/repo_tools/smoke-workflow.sh` + payload‑копии, `dev/doc/customization.md`.
- [x] DoD: дефолты профилей по событиям действуют, stdout короткий, полный лог сохраняется.

### Phase 2: Расширение pack + events + index
- [x] Расширить pack на QA/PRD/Review: обновить `src/claude_workflow_cli/tools/{qa_agent.py,prd_review.py}` для sidecar pack и обновить схемы в `dev/doc/reports-format.md`.
- [x] Columnar‑секции: findings (QA/PRD/Review), matches/reuse/call_graph/import_graph (research) с `cols/rows` и reference‑таблицами в `dev/doc/reports-format.md`.
- [x] Pack‑first чтение вне tasks‑derive: обновить `aidd/{agents,commands}/*.md`, `dev/doc/agents-playbook.md`, `dev/doc/prompt-playbook.md`; sync `scripts/sync-payload.sh --direction=from-root`.
...
- [x] Линт/гейты: расширить `dev/repo_tools/lint-prompts.py`, `dev/tests/test_prompt_lint.py` и добавить `dev/tests/test_index_schema.py` для schema index.
...
- [x] RFC6902 patch updates: `dev/repo_tools/apply_json_patch.py`, `reports/<type>/<ticket>.patch.json`, флаг `--emit-patch` в `qa_agent.py`/`prd_review.py` — включать только при доказанном pain (diff/size).
- [x] TOON optional: конвертер `full.json -> pack.toon` под `AIDD_PACK_FORMAT=toon`, ограничения в `dev/doc/reports-format.md`.
...
- [ ] `src/claude_workflow_cli/data/payload/aidd/hooks/format-and-test.sh`: вынести `COMMON_PATTERNS`/`DEFAULT_CODE_FILES` в `.claude/settings.json` (новые ключи) и добавить дефолты для npm/py/go/rust/.NET; обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/customization.md`.
...
### Docs & examples
- [ ] Решить судьбу Gradle‑демо: оставить как optional example с явным маркированием или добавить language‑agnostic demo и вынести `examples/gradle-demo` в legacy; обновить `README.md`, `README.en.md`, `dev/doc/distro-audit.md`.
...
### Tests
- [ ] Добавить тесты/смоук для новых config‑ключей (`format-and-test`, `lint-deps`, init‑скрипт) и обновить `dev/tests/test_init_aidd.py`/`src/claude_workflow_cli/data/payload/aidd/dev/repo_tools/smoke-workflow.sh` при необходимости.
...
- [ ] Обновить контракт команд в `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md`: добавить `/flow` в матрицу (входы/выходы), описать политику `SlashCommand` и handoff‑правила.
...
- [ ] Обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/release-notes.md`, `CHANGELOG.md`, `src/claude_workflow_cli/data/payload/manifest.json` после добавления команды.
- [ ] Прогнать `python3 dev/repo_tools/lint-prompts.py --root src/claude_workflow_cli/data/payload/aidd` и `python3 tools/check_payload_sync.py`, затем синхронизировать payload → root: `scripts/sync-payload.sh --direction=to-root`.
...
### Тесты и CI
- [x] Обновить smoke: `src/claude_workflow_cli/data/payload/aidd/dev/repo_tools/smoke-workflow.sh` (или заменить на вызов `claude-workflow smoke`) без обращения к `tools/*.py`/`scripts/*.py`.
- [x] Переписать unit‑тесты на CLI‑first: `dev/tests/test_init_aidd.py`, `dev/tests/test_gate_workflow.py`, `dev/tests/test_gate_qa.py`, `dev/tests/test_prd_review_gate.py`, `dev/tests/test_prompt_versioning.py` (если зависит от скриптов).
- [x] Добавить тесты наличия новых CLI‑подкоманд и отсутствия legacy‑файлов в payload.

### Документация и релиз
- [x] Обновить `README.md`, `README.en.md`, `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/agents-playbook.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md` — заменить ссылки на `tools/*.py`/`scripts/*.py` на `claude-workflow`.
- [x] Обновить `src/claude_workflow_cli/data/payload/aidd/dev/doc/release-notes.md` и `CHANGELOG.md` (breaking change: legacy scripts removed).
- [x] Финал: `python3 tools/check_payload_sync.py`, `scripts/sync-payload.sh --direction=to-root`, полный прогон `dev/repo_tools/ci-lint.sh`.
...
- [ ] Ввести единый helper `hook_project_root` в `src/claude_workflow_cli/data/payload/aidd/hooks/lib.sh` с приоритетом: `CLAUDE_PLUGIN_ROOT` → `git -C "$PWD" rev-parse --show-toplevel` → поиск вверх `aidd/docs`; возвращать абсолютный путь (realpath) и пустое значение при провале.
...
- [ ] Унифицировать резолвинг путей в runtime‑скриптах и tools: добавить `--root` и единый helper `get_root()` в `src/claude_workflow_cli/data/payload/aidd/scripts/{qa-agent.py,prd-review-agent.py,plan_review_gate.py,prd_review_gate.py}` и `src/claude_workflow_cli/data/payload/aidd/tools/{set_active_stage.py,set_active_feature.py,run_cli.py}`; исключить относительные `./aidd` пути.
- [ ] Обновить документацию об источнике root и запуске CLI: `src/claude_workflow_cli/data/payload/aidd/workflow.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/agents-playbook.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/prompt-playbook.md` (правило: хуки и CLI работают от workspace root).
- [ ] Добавить тесты на резолвинг root: эмуляция `CLAUDE_PLUGIN_ROOT` (cache path), запуск hook из чужого cwd; отдельный кейс non‑git — ожидается мягкий exit. Файлы: `dev/tests/test_gate_workflow.py`, `dev/tests/helpers.py`.
...
- [ ] `claude-workflow init --type {claude-code-plugin|open-code|both}` + help/usage. **Deps:** нет. **AC:** default = `claude-code-plugin`; `--type open-code` не создаёт `.claude*`; `--type both` создаёт оба overlay; `--dry-run` отражает выбранный тип. Файлы: `src/claude_workflow_cli/cli.py`, `src/claude_workflow_cli/data/payload/aidd/init-claude-workflow.sh`.
- [ ] Разделить генерацию overlay в init‑скрипте (Claude Code vs OpenCode). **Deps:** флаг `--type`. **AC:** `opencode.json` и `.opencode/**` появляются в workspace root; `.claude/` и `.claude-plugin/` не создаются при `--type open-code`; логирование явно указывает выбранный тип. Файл: `src/claude_workflow_cli/data/payload/aidd/init-claude-workflow.sh`.
- [ ] Обновить root‑детекцию CLI под OpenCode. **Deps:** init‑флаги. **AC:** команды, использующие `_require_workflow_root`, работают при наличии только `opencode.json`/`.opencode/**`; `WORKSPACE_ROOT_DIRS` учитывает `.opencode` и `opencode.json` (или спец‑обработку файла); ошибки упоминают оба типа установки; `sync`/`upgrade` не переносят `opencode.json` внутрь `aidd/`; `tools/check_payload_sync.py` считает `opencode.json` workspace‑root артефактом. Файлы: `src/claude_workflow_cli/cli.py`, `tools/check_payload_sync.py`.
- [ ] Обновить `claude-workflow sync/upgrade` под OpenCode. **Deps:** root‑детекция. **AC:** `sync` по умолчанию подтягивает `.opencode/` и `opencode.json` при open‑code установке (или доступен `--include opencode`), `upgrade` корректно раскладывает workspace‑root артефакты, `--dry-run` отражает opencode‑файлы. Файл: `src/claude_workflow_cli/cli.py`.
- [ ] Зафиксировать источник `automation` для OpenCode и внедрить fallback. **Deps:** init‑изменения. **AC:** `format-and-test.sh` работает без `.claude/settings.json`; приоритет источников задокументирован (например, `CLAUDE_SETTINGS_PATH`/`.claude/settings.json` → `aidd/config/automation.json`); выбранный источник создаётся при `--type open-code` или доступен по умолчанию. Файлы: `src/claude_workflow_cli/data/payload/aidd/hooks/format-and-test.sh`, `src/claude_workflow_cli/data/payload/opencode.json` (+ выбранный конфиг).

### OpenCode overlay (payload)
- [ ] Добавить `opencode.json` в payload root. **Deps:** init‑разделение overlay. **AC:** `$schema: https://opencode.ai/config.json`, `instructions: ["aidd/AGENTS.md"]`, заданы `permission`/`tools`, опционально `model`/`default_agent`; permission позволяет вызывать `claude-workflow` и `aidd/hooks/*.sh` без лишних подтверждений; способ регистрации plugin/commands/agents подтверждён (если требуется — перечислить пути в `opencode.json`, иначе зафиксировать автоподхват в документации); файл валиден относительно schema (тест/линт); файл включён в manifest. Файл: `src/claude_workflow_cli/data/payload/opencode.json`.
- [ ] `.opencode/command/*.md` thin‑wrappers на `@aidd/commands/<name>.md`. **Deps:** opencode.json. **AC:** покрыты команды `idea-new`, `researcher`, `plan-new`, `review-spec`, `tasks-new`, `implement`, `review`, `qa`, `status`; frontmatter содержит `description`, `agent`, `model`; без дублирования контента. Файлы: `src/claude_workflow_cli/data/payload/.opencode/command/*.md`.
- [ ] `.opencode/agent/*.md` thin‑wrappers на `@aidd/agents/<name>.md`. **Deps:** opencode.json. **AC:** покрыты агенты `analyst`, `researcher`, `planner`, `validator`, `plan-reviewer`, `prd-reviewer`, `implementer`, `reviewer`, `qa`; frontmatter содержит `description`, `mode: subagent`, `model`, `tools`/`permission`. Файлы: `src/claude_workflow_cli/data/payload/.opencode/agent/*.md`.

### OpenCode plugin: гейты и авто‑проверки
- [ ] Реализовать `.opencode/plugin/aidd-workflow.ts`. **Deps:** root‑детекция (Wave 64) + automation‑решение. **AC:** `tool.execute.before` блокирует изменения `src/**` вне стадий `implement|review|qa` (только когда инструмент явно трогает путь под `src/`); при отсутствии root — warn и пропуск; `session.idle`/`tool.execute.after` вызывает `aidd/hooks/{gate-workflow,gate-tests,gate-qa,format-and-test,lint-deps}.sh` последовательно; передаётся `CLAUDE_PLUGIN_ROOT`; non‑code изменения не блокируются. Файл: `src/claude_workflow_cli/data/payload/.opencode/plugin/aidd-workflow.ts`.
- [ ] Формализовать exit‑codes/сообщения для OpenCode plugin. **Deps:** базовая реализация plugin. **AC:** soft/hard‑блокировки соответствуют хукам (`exit 2` блокирует, `exit 0` пропускает, прочие — warn без блокировки); сообщения совпадают по стилю с hook‑скриптами и содержат причину/следующее действие; константы вынесены в начале файла. Файл: `src/claude_workflow_cli/data/payload/.opencode/plugin/aidd-workflow.ts`.
- [ ] Добавить context‑gc hooks в OpenCode plugin. **Deps:** базовая реализация plugin. **AC:** аналоги `SessionStart`, `PreCompact`, `PreToolUse`, `UserPromptSubmit` вызывают `claude-workflow context-gc {sessionstart,precompact,pretooluse,userprompt}`; если часть событий недоступна в OpenCode — явно документировать ограничения. Файлы: `src/claude_workflow_cli/data/payload/.opencode/plugin/aidd-workflow.ts`, `dev/doc/workflow.md`.

### Packaging/sync/audit
- [ ] Обновить payload‑манифест/allowlist/синк. **Deps:** новые файлы overlay. **AC:** `manifest.json` содержит `opencode.json` и `.opencode/**`; `tools/payload_audit_rules.json` разрешает новые пути; `scripts/sync-payload.sh` синхронизирует `opencode.json` и `.opencode/` по умолчанию (и/или `--include opencode`); `tools/check_payload_sync.py` проверяет новые корневые пути. Файлы: `src/claude_workflow_cli/data/payload/manifest.json`, `tools/payload_audit_rules.json`, `scripts/sync-payload.sh`, `tools/check_payload_sync.py`.
- [ ] Обновить packaging для dotfiles. **Deps:** обновлённый payload. **AC:** wheel и payload‑архив содержат `opencode.json` и `.opencode/**`; `pyproject.toml`/`MANIFEST.in` покрывают новые dot‑директории. Файлы: `pyproject.toml`, `MANIFEST.in`, `dev/tests/test_package_payload.py`.

### Tests
- [ ] Тесты `init --type open-code` и `--type both`. **Deps:** init + overlay. **AC:** создаются `opencode.json`, `.opencode/{command,agent,plugin}`; `.claude/` и `.claude-plugin/` отсутствуют при `open-code`; CLI‑команды работают при open‑code‑only установке; `sync/upgrade` корректно обрабатывают workspace‑root `opencode.json`. Файлы: `dev/tests/test_init_aidd.py` (или новый `dev/tests/test_init_open_code.py`), `dev/tests/test_cli_sync.py`.
- [ ] Обновить `dev/tests/test_package_payload.py`. **Deps:** packaging update. **AC:** проверки ожидают `opencode.json` и `.opencode/**` в wheel/payload‑zip.
- [ ] Добавить проверку `opencode.json` и wrapper‑файлов. **Deps:** opencode overlay. **AC:** `opencode.json` валиден по schema (или минимальный контракт проверен тестом); wrapper‑файлы ссылаются на `@aidd/commands/*` и `@aidd/agents/*` без дублирования контента. Файлы: `dev/tests/test_package_payload.py` (или отдельный тест).
- [ ] Добавить smoke‑сценарий OpenCode. **Deps:** overlay + plugin. **AC:** сценарий `init --type open-code` создаёт `opencode.json`/`.opencode`, не создаёт `.claude*`, проверяет базовые гейты/хуки (dry‑run или с фиктивным payload) и корректные exit‑codes. Файлы: `dev/repo_tools/smoke-workflow.sh` (или новый smoke‑скрипт), `src/claude_workflow_cli/data/payload/smoke-workflow.sh` (если применимо).

### Документация
- [ ] README + dev‑гайды под dual‑deploy. **Deps:** init/overlay/automation. **AC:** `README.md`/`README.en.md` показывают `--type` и layout (`.claude*` vs `.opencode*`); `dev/doc/workflow.md` и `dev/doc/customization.md` описывают OpenCode‑режим, plugin‑гейты, источник automation и зависимость от установленного `claude-workflow` для хуков; отсутствуют ссылки на несуществующие payload‑доки. Файлы: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/customization.md`.
...
- [x] Обновить `dev/doc/agents-playbook.md`: новая политика тестов, лимит итерации, `test-policy.env`, примеры Gradle `--tests`, правила повторного прогона.
- [x] Обновить `dev/doc/customization.md`: новые env/config ключи (`AIDD_TEST_PROFILE`, `AIDD_TEST_FILTERS`, `AIDD_TEST_TASKS`, `AIDD_TEST_FORCE`, `fastTasks/fullTasks/targetedTask`), `aidd/.cache/test-policy.env` примеры, dedupe/budget.
...
- [x] Добавить запись в `dev/doc/release-notes.md`, `CHANGELOG.md` и (при необходимости) `README.md`/`README.en.md` о новой политике тестов/итераций.

### Тесты и синхронизация
- [x] Создать новые тест-кейсы (в `dev/tests/test_format_and_test.py` и/или отдельном модуле) на `AIDD_TEST_PROFILE` (fast/targeted/full/none), чтение `test-policy.env`, `AIDD_TEST_FILTERS/TASKS`, dedupe‑скип, `AIDD_TEST_FORCE`, default fast, новый конфиг профилей.
- [x] Обновить smoke-сценарии: `dev/repo_tools/smoke-workflow.sh` и `src/claude_workflow_cli/data/payload/smoke-workflow.sh` — добавить проверки нового `Test profile`/policy и отсутствия повторного прогона при dedupe.
- [x] Финал: `dev/repo_tools/prompt-version` (bump RU), `dev/repo_tools/lint-prompts.py --root <root>`, `python3 tools/check_payload_sync.py`, `scripts/sync-payload.sh --direction=to-root`.
...
- [x] Зафиксировать карту установки (uv tool install → `claude_workflow_cli/data/payload/**` → `claude-workflow init`): обновить `dev/doc/distro-audit.md` и таблицу “core vs dev-only”, без упоминания пресетов и EN‑промптов.
...
- [x] Удалить `prompts/en/**`: убрать `--prompt-locale` из CLI и init‑скрипта, удалить паритет‑проверки из `gate-workflow.sh`, пересмотреть `dev/repo_tools/lint-prompts.py`, обновить `dev/doc/prompt-versioning.md` и `dev/doc/prompt-playbook.md`, удалить упоминания из README/тестов.
- [x] Аудит необходимости `aidd/dev/doc/customization.md` и `aidd/workflow.md`: либо сохранить как core user‑guides, либо перенести в `dev/doc/**` и удалить из payload/manifest/tests.

### Нормализация шаблонов (templates)
- [x] Перенести шаблоны артефактов в соответствующие подпапки: `docs/prd/template.md` → `docs/prd/template.md`, `docs/adr/template.md` → `docs/adr/template.md`, `docs/tasklist/template.md` → `docs/tasklist/template.md`, `docs/research/template.md` → `docs/research/template.md`; обновить ссылки в `dev/doc/workflow.md`, `agents/commands`, `init-claude-workflow.sh`, README и тестах.
...
- [x] Определить финальное место для `dev/doc/templates/prompts/prompt-agent.md` и `dev/doc/templates/prompts/prompt-command.md` (например, `agents/templates/` и `commands/templates/` или `dev/doc/templates/prompts/`), перенести и обновить `scripts/scaffold_prompt.py`, `dev/doc/prompt-playbook.md`, README.

### Docs: runtime vs dev-only
- [x] Разделить maintainer‑инструкции: вынести repo-only шаги из `aidd/dev/doc/release-notes.md`, `aidd/dev/doc/prompt-versioning.md`, `aidd/dev/doc/prompt-playbook.md` в `dev/doc/**`; в payload оставить user‑facing версии без ссылок на `scripts/*` и `tools/*` из корня.
- [x] Если `aidd/dev/doc/customization.md` и `aidd/workflow.md` остаются в payload: заменить repo-only команды на CLI аналоги или пометки “repo-only”; убедиться, что payload‑доки не ссылаются на отсутствующие файлы.
...
- [x] Определить dev-only местоположение и консолидировать версии: `aidd/dev/doc/agents-playbook.md`, `aidd/dev/doc/feature-cookbook.md`, `aidd/dev/doc/qa-playbook.md`, `aidd/dev/doc/migration-aidd.md`, `aidd/dev/doc/prompt-playbook.md`, `aidd/dev/doc/prompt-versioning.md`, `aidd/dev/doc/release-notes.md` → `dev/doc/**` (свести дубли, сохранить историю).
- [x] Удалить payload-копии и заменить/удалить ссылки в payload: `aidd/AGENTS.md`, `aidd/docs/tasklist/template.md`, `aidd/agents/dev/doc/templates/prompts/prompt-agent.md`, `aidd/commands/dev/doc/templates/prompts/prompt-command.md`, а также любые упоминания в `aidd/docs/*`, чтобы payload не ссылался на dev-only файлы.
...
- [x] Обновить инвентаризацию/пакетирование: `src/claude_workflow_cli/data/payload/manifest.json`, `tools/payload_audit_rules.json`, `tools/check_payload_sync.py`, `scripts/sync-payload.sh`, `dev/doc/distro-audit.md`.
- [x] Обновить тесты пакета: `dev/tests/test_package_payload.py`, `dev/tests/test_bootstrap_e2e.py` (и при необходимости регрессию на отсутствие dev-only ссылок).
...
- [x] Обновить тесты (`dev/tests/test_package_payload.py`, `dev/tests/test_bootstrap_e2e.py`, `dev/tests/test_gate_workflow.py` при необходимости) под новые пути и удалённые каталоги; добавить регрессию на отсутствие repo-only ссылок в payload docs.
...
- [x] Зафиксировать канонический путь working set и использовать его в `aidd/AGENTS.md`, `src/claude_workflow_cli/context_gc/working_set_builder.py`, `dev/doc/agents-playbook.md`.
...
- [x] Обновить `dev/tests/test_format_and_test.py` и smoke‑сценарии (`dev/repo_tools/smoke-workflow.sh`, `src/claude_workflow_cli/data/payload/smoke-workflow.sh`) под дефолты Stop/SubagentStop и summary‑логи.
- [x] Обновить `dev/doc/customization.md` и `dev/doc/agents-playbook.md`: anchors‑first/pack‑first, `AIDD:CONTEXT_PACK`, новые дефолты профиля и формат логов.
...
- [x] Добавить `aidd/docs/tickets/` и шаблон `aidd/docs/tickets/template.yaml` (schema v1: ticket/slug/stage/status/owners/artifacts/dev/tests/reports).
...
- [x] Линт schema + smoke: `dev/tests/test_ticket_manifest.py`, обновить `dev/repo_tools/lint-prompts.py`/`dev/tests/test_prompt_lint.py` для наличия manifest.
...
- [x] Зафиксировать schema header для отчетов в `dev/doc/reports-format.md` (ticket/stage/status/started_at/finished_at/tool_versions/summary).
...
- [x] Тесты на guard‑policy: `dev/tests/test_context_gc.py` + smoke‑сценарии.
...
- [x] Документация и примеры: `dev/doc/workflow.md`, `dev/doc/customization.md`, обновить `manifest.json`.

### EPIC H — Prompt slimming + mkdir-free + hook noise
- [x] Slim команды: ограничить размер `/commands/*.md` (≤160 строк), убрать повторяющиеся блоки политики, оставить ссылки на `aidd/AGENTS.md` и stage‑anchors; обновить `dev/repo_tools/lint-prompts.py`/`dev/tests/test_prompt_lint.py` с проверкой лимита.
...
- [x] Привести ссылки в `aidd/commands/*.md`, `aidd/agents/*.md`, `aidd/docs/anchors/*.md`, `aidd/docs/*template.md`, `dev/doc/reports-format.md`, `README*.md`, `dev/doc/*`, smoke‑скриптах и payload‑копиях к `aidd/reports/**`.
...
### EPIC D — Lint/dev/tests/sync
- [x] Обновить `dev/repo_tools/lint-prompts.py`, `dev/tests/test_prompt_lint.py`, `dev/repo_tools/upgrade_aidd_docs.py` под якоря `AIDD:HANDOFF_INBOX`, `AIDD:RESEARCH_HINTS`, `AIDD:ACCEPTANCE`, `AIDD:RISKS` (убрать `AIDD:INBOX_DERIVED`, `AIDD:ACCEPTANCE_CRITERIA`, `AIDD:CONTRACTS`, `AIDD:OBSERVABILITY`, `AIDD:GAPS`, `AIDD:RISKS_TOP5`).
...
### Tests и docs
- [x] Тесты на QA handoff. **Deps:** автогенерация QA handoff. **AC:** `tasks-derive --source qa` пишет в `AIDD:HANDOFF_INBOX`; `gate-qa`/`gate-workflow` реагируют на отсутствие записи; добавлены unit‑тесты и smoke‑сценарий. Файлы: `dev/tests/test_tasks_derive.py` (или новый), `dev/repo_tools/smoke-workflow.sh`.
- [x] Тесты на review handoff. **Deps:** review report + tasks-derive review. **AC:** отчёт ревью → задачи в handoff; `gate-workflow` блокирует при отсутствии записей. Файлы: `dev/tests/test_tasks_derive.py`, `dev/tests/test_gate_workflow.py`.
- [x] Тесты на повторные запуски QA/Review. **Deps:** идемпотентность handoff. **AC:** двойной `tasks-derive` не добавляет дубликатов, обновляет существующие задачи по id; smoke‑сценарий проверяет повторный `/feature-dev-aidd:qa` и `/feature-dev-aidd:review`. Файлы: `dev/tests/test_tasks_derive.py`, `dev/repo_tools/smoke-workflow.sh`.
- [x] Документация. **Deps:** все выше. **AC:** anchors `aidd/docs/anchors/qa.md` и `aidd/docs/anchors/review.md`, а также `/feature-dev-aidd:qa` и `/feature-dev-aidd:review` описывают auto‑handoff и формат источников; `reports-format.md` отражает review report. Файлы: `src/claude_workflow_cli/data/payload/aidd/docs/anchors/{qa,review}.md`, `src/claude_workflow_cli/data/payload/aidd/dev/doc/reports-format.md`, `README.md`.
...
- [x] Обновить `dev/repo_tools/smoke-workflow.sh` под spec‑file и новые секции tasklist.
...
- [x] Расширить `tasklist-check`: валидировать `iteration_id` в `AIDD:ITERATIONS_FULL`/`AIDD:NEXT_3` и покрытие `AIDD:ITERATIONS` из плана. **AC:** check падает при отсутствии `iteration_id` или несовпадении с планом. Файлы: `src/claude_workflow_cli/tools/tasklist_check.py`, `aidd/scripts/tasklist-check.py`, тесты `dev/tests/test_tasklist_check.py` (или новый).
...
- [x] Обновить `tasklist-check` и lint: требовать `AIDD:TEST_EXECUTION` + базовые поля (profile/tasks/filters/when/reason). **AC:** lint и check валидируют новую секцию. Файлы: `src/claude_workflow_cli/tools/tasklist_check.py`, `dev/repo_tools/lint-prompts.py`, `dev/tests/test_prompt_lint.py`.
...
- [x] Вынести workspace‑шаблоны в `templates/aidd/` (копии `aidd/docs`, `aidd/reports`, `aidd/config`, `.markdownlint.yaml`, `conventions.md`, `AGENTS.md`). **AC:** шаблоны не лежат в plugin root, а используются только для init.
...
- [x] Переписать `README.md`/`README.en.md`/`dev/doc/workflow.md` под поток `/plugin marketplace add` → `/plugin install` → `/feature-dev-aidd:aidd-init`. **AC:** нет упоминаний `claude-workflow init/sync/upgrade`.
- [x] Обновить `dev/doc/design/install-subdir.md` и ADR: зафиксировать marketplace‑only и отказ от CLI‑установки. **AC:** дизайн/ADR синхронизированы с новой моделью.
...
- [x] Обновить smoke/dev/tests/docs под новый путь запуска и плоский layout (включая `dev/repo_tools/smoke-workflow.sh`, тесты CLI, README/дев‑доки). **AC:** `dev/repo_tools/ci-lint.sh` и `dev/repo_tools/smoke-workflow.sh` проходят, упоминаний `scripts/aidd_*.py` нет.

### EPIC G — Python‑entrypoint hook‑скрипты (вызов как `.sh`)
- [x] Зафиксировать стандарт: hook‑скрипты остаются в `hooks/*.sh`, но реализованы на Python с shebang (`#!/usr/bin/env python3`) и вызываются как `${CLAUDE_PLUGIN_ROOT}/hooks/*.sh`. **AC:** стандарт описан в `dev/doc/workflow.md`/`dev/doc/customization.md`, пример обновлён в `hooks/format-and-test.sh`.
...
- [x] Переписать `hooks/context-gc-*.sh` как Python‑entrypoints (precompact/sessionstart/pretooluse/userprompt/stop) и перенести логику в `hooks/context_gc/`. **AC:** `hooks/hooks.json` вызывает только `hooks/context-gc*.sh`; `dev/tests/test_context_gc.py` и smoke проходят.
- [x] Переписать `hooks/gate-workflow.sh` в Python‑entrypoint: analyst‑check, plan/prd review, research checks, tasklist‑progress, reviewer marker, handoff. **AC:** `rg "bash" hooks/gate-workflow.sh` не находит bash‑логики; `dev/tests/test_gate_workflow.py`/`dev/tests/test_gate_researcher.py` проходят.
...
- [x] Переписать `hooks/gate-tests.sh` в Python‑entrypoint: gates config, test rules, reviewer marker, research‑targets checks. **AC:** `dev/tests/test_gate_tests_hook.py` проходит; выходы и exit‑коды совпадают с текущими.
- [x] Переписать `hooks/gate-qa.sh` в Python‑entrypoint: QA‑агент (логика из `tools/qa_agent.py`), отчёт, handoff, debounce. **AC:** `dev/tests/test_gate_qa.py` проходит; формат `aidd/reports/qa/*.json` совместим.
- [x] Переписать `hooks/gate-prd-review.sh` в Python‑entrypoint (или удалить при отсутствии точки вызова). **AC:** нет висячих hook‑скриптов; `dev/tests/test_gate_prd_review.py` синхронизирован.
- [x] Переписать `hooks/format-and-test.sh` и `hooks/lint-deps.sh` в Python‑entrypoints с сохранением поведения (`AIDD_TEST_*`, deps allowlist). **AC:** `dev/tests/test_format_and_test.py` и `dev/tests/test_post_hook_paths.py` проходят.
...
- [x] Зафиксировать директорию runtime‑entrypoints: `tools/*.sh` (только в plugin root, без копий в `templates/aidd/`) и базовый шаблон скрипта (shebang, **строгое** требование `CLAUDE_PLUGIN_ROOT`, запуск логики напрямую). **AC:** стандарт зафиксирован в `dev/doc/workflow.md`, примеры обновлены.
- [x] Сгенерировать entrypoints для всех runtime‑команд: `init`, `smoke`, `set-active-feature`, `identifiers`, `set-active-stage`, `prd-review`, `plan-review-gate`, `prd-review-gate`, `tasklist-check`, `researcher-context`, `analyst-check`, `research-check`, `research`, `reviewer-tests`, `review-report`, `tasks-derive`, `context-pack`, `index-sync`, `status`, `tests-log`, `qa`, `progress`. **AC:** каждая команда доступна как `${CLAUDE_PLUGIN_ROOT}/tools/<command>.sh`.
...
- [x] Уточнить `tasks-derive`: вставка handoff‑блоков под `AIDD:HANDOFF_INBOX`, merge по `id`/signature и корректные задачи QA/Research (`QA tests: fail`, recommendations, reuse candidates). **AC:** `dev/tests/test_tasks_derive.py` проходит.
...
- [x] Обновить документацию и шаблоны: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `dev/doc/cli-migration.md`, `dev/doc/qa-playbook.md`, `templates/aidd/conventions.md`, `templates/aidd/docs/prd/template.md`. **AC:** в docs нет упоминаний `python3 -m` как основного способа вызова.
...
- [x] Обновить тесты и smoke: `dev/tests/helpers.py`, `dev/tests/test_init_aidd.py`, `dev/tests/test_research_command.py`, `dev/tests/test_cli_subcommands.py`, `dev/repo_tools/smoke-workflow.sh` — переход на новые runtime‑entrypoints. **AC:** тесты/смоук вызывают только entrypoint‑скрипты.
- [x] Удалить устаревший runtime‑пакет целиком и очистить все документы/подсказки от упоминаний устаревшего CLI (оставить только `tools/*.sh`). **AC:** `rg` не находит совпадений старых runtime‑путей в доках/коде; `CHANGELOG.md`/`dev/doc/release-notes.md` отражают удаление.

### EPIC I — Marketplace install hardening
- [x] Выровнять команды с namespace‑правилами плагинов или явно задокументировать alias‑сценарий. **AC:** `README.md`, `README.en.md`, `dev/doc/workflow.md`, `commands/*.md` содержат корректные примеры (`/feature-dev-aidd:<command>` или описанный alias), новые пользователи не получают `unknown command`. Файлы: `README.md`, `README.en.md`, `dev/doc/workflow.md`, `commands/*.md`.
