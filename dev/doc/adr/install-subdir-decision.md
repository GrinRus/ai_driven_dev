...
- `/feature-dev-aidd:aidd-init` создаёт структуру из `templates/aidd` и не перезаписывает существующие файлы.
...
- Запуск хуков вне workspace должен явно падать с подсказкой `/feature-dev-aidd:aidd-init`.

## Приложение: структура и сценарии установки
...
- При нестандартном размещении плагина используйте `CLAUDE_PLUGIN_ROOT` для ручного запуска.

### Миграция из корня
См. `dev/doc/migration-aidd.md` для пошаговой инструкции и списка переносимых артефактов.
