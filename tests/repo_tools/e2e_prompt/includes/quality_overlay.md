...
- `$PROMPT_RENDER_DIR/aidd_test_flow_prompt_ralph_script_full.txt`
...
- `TICKET=TST-002`
- `PROMPT_RENDER_DIR=$PROJECT_DIR/.aidd_audit/$TICKET/rendered-prompts`
- `BASE_PROMPT=$PROMPT_RENDER_DIR/aidd_test_flow_prompt_ralph_script_full.txt`
...
1. Render the prompt set once: `python3 $PLUGIN_DIR/tests/repo_tools/build_e2e_prompts.py --output-dir "$PROMPT_RENDER_DIR"`.
2. Run the inherited flow steps using `BASE_PROMPT`.
3. Evaluate final feature state, code quality, artifact quality, and acceptance traceability.
4. Produce quality verdict and optional backlog wave.
5. End with the exact final marker:
