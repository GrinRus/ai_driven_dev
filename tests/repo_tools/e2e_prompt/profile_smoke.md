...
- R12.4: Если `review-spec` narrative расходится с `aidd/reports/prd/<ticket>.json|*.pack.json` по числу/типу findings при валидном structured report payload, фиксировать `INFO(review_spec_report_mismatch_telemetry)` и принимать recovery-решение только по report payload.
- R12.4a: Если при `review_spec_report_mismatch` structured report payload отсутствует/невалиден, фиксировать `prompt-exec issue (review_spec_report_mismatch)`.
...
- Если в QA orchestration/логе встречается direct host-absolute runtime path `python3 /.../skills/*/runtime/*.py` (включая nested tool-calls), классифицировать как `prompt-flow drift (non-canonical runtime path)`; использовать только `${CLAUDE_PLUGIN_ROOT}`/`$PLUGIN_DIR` canonical runtime paths.
- Если QA flow предлагает manual write path `stage.*.result.json`/direct `stage_result.py` write как recovery, классифицировать как `policy_violation(stage_result_manual_write)` и не выполнять manual path.
...
- Классификация workspace-layout:
  - `INFO(preexisting_noncanonical_root)` для pre-existing root-path без delta в рамках run;
  - `WARN(workspace_layout_non_canonical_root_detected)` только при фактической мутации в рамках run.
