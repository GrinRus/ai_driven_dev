...
- R2: Если `BLOCKED` связан с unresolved `Q*`:
  ...
  - если PRD остаётся `Status != READY` при `open_questions_count=0`, выполнить findings-sync cycle (`idea-new` и/или `plan-new` с `AIDD:SYNC_FROM_REVIEW ...`) и затем повторить `review-spec`;
  - если после findings-sync PRD остаётся `Status != READY`, классифицировать как `NOT VERIFIED (findings_sync_not_converged)` + `prompt-flow gap` и не повторять stage, требующий `PRD READY`.
...
- R7.1: Если в UI/операторском выводе встречается unprefixed alias (`/idea-new` и т.п.), но в `init.slash_commands` присутствуют `feature-dev-aidd:*`, фиксировать `INFO(prefix_alias_display_only)`; использовать canonical prefixed команды.
...
- R9.1: Для `cwd_wrong` authoritative-source = launcher topology precheck + stage summary/termination/preflight evidence; упоминания `cwd_wrong` в nested excerpts/tool_result считать telemetry-only и не использовать как primary классификацию.
- R10: Если stage-return уводит в manual path (direct вызов внутреннего stage-chain preflight entrypoint или ручная запись `stage.*.result.json`) — классифицировать как `prompt-flow drift`, manual path не выполнять.
...
- R10.2: Если в stage-логе есть direct вызов внутреннего stage-chain preflight entrypoint вне canonical stage-chain — классифицировать как `prompt-flow drift (non-canonical stage orchestration)` и завершать шаг `NOT VERIFIED`.
...
- R11.1: Для `loop-run` в `text`-режиме top-level result может приходить как JSON event `{"type":"result","schema":"aidd.loop_result.v1",...}`; учитывать его как валидный terminal result.
...
- R11.5: Source-of-truth для test execution policy в runtime = `aidd/config/gates.json`; использование `.claude/settings.json`/`CLAUDE_SETTINGS_PATH` в runtime decision path классифицировать как `prompt-flow drift (non-canonical test policy source)`.
- R11.6: Если одновременно зафиксированы `reason_code=project_contract_missing|tests_cwd_mismatch` и `no_top_level_result`, reason-code считать primary причиной, а `no_top_level_result` — secondary symptom.
- R11.7: Для non-research стадий (`idea|plan|review-spec|tasks|implement|review|qa|loop`) source-of-truth по retry/gates = текущий top-level stage-return + canonical report payload; nested excerpts/tool_result/persisted template blocks считать telemetry и не использовать как primary trigger.
- R11.8: Контекст-артефакты (`aidd/reports/context/*.md`, `latest_working_set*.md`) должны быть compact summary без template body leakage; при contamination ожидать `hard_replace` + WARN telemetry.
- R11.9: Любые recovery-предложения с `AIDD_ALLOW_PLUGIN_WORKSPACE=1` считать non-canonical bypass (`prompt-flow drift (workspace_bypass_attempt)`); такой path не выполнять.
- R11.10: Self-diagnosis recovery через чтение runtime source (`skills/*/runtime/*.py`) запрещён как primary path; диагностика/ретраи только по top-level stage-return + canonical report payload.
- R11.11: Source-of-truth для research status/gates = текущий top-level stage-return + canonical research report payload (`aidd/reports/research/*.pack.json`); nested excerpts/tool_result/persisted template blocks — telemetry-only.
- R11.12: Ручной status-upgrade drift запрещён: не повышать `warn|pending` до `ready|reviewed` вручную. Любые такие рекомендации фиксировать как `prompt-flow drift (research_status_manual_upgrade_attempt)`.
- R11.13: Gate-read canonicalization: `research Status: ready` трактуется как alias `reviewed` с telemetry `research_status_alias_normalized=ready->reviewed`; alias сам по себе не является blocker.
...
- R12.1c: Для readiness gate `research_status=ready` канонизируется в `reviewed`; `warn` остаётся soft/non-terminal при наличии minimal RLM baseline и требует telemetry `research_gate_softened=true`.
...
- R12.2: При `FAIL` readiness gate обязателен `reason_code` из набора `prd_not_ready|open_questions_present|answers_format_invalid|research_not_ready`; для `prd_not_ready|open_questions_present|answers_format_invalid|research_not_ready` перед terminal-классификацией обязателен ровно один readiness-recovery цикл:
  - для `prd_not_ready|open_questions_present|answers_format_invalid`: question-closure (`idea-new` и `plan-new` compact retry при валидном trigger) -> `/feature-dev-aidd:review-spec <ticket>` -> пересчёт `05_precondition_block.txt`;
...
- R12.5: Если `review-spec` вернул `WARN|NEEDS_REVISION`, unresolved `Q*` отсутствуют, запускать findings-sync cycle:
  ...
  - после sync-retry повторить `/feature-dev-aidd:review-spec <ticket>` и пересчитать `05_precondition_block.txt`.
- R12.6: Если после findings-sync cycle `readiness_gate` остаётся `FAIL`, классифицировать как `NOT VERIFIED (findings_sync_not_converged)` + `prompt-flow gap` и не запускать downstream stage.
...
- Если bootstrap probe содержит `ModuleNotFoundError: No module named 'aidd_runtime'`:
  ...
  - продолжать smoke аудит разрешено, но любой fallback/probe через direct runtime path маркировать `NOT VERIFIED (runtime_bootstrap_missing)`.
...
- manual internal stage-chain path не использовать как recovery для slash stage-команд 5.x.
...
- После retry повторно снять `05_prd_head.txt` и зафиксировать актуальный `Status` + unresolved `Q*`.
...
- Артефакты:
  ...
  - `05_plan_head.txt`
- Если после retry вопросы плана остаются незакрытыми по top-level stage-return:
  - шаг 5.3 = `NOT VERIFIED (plan_qna_unresolved)` + `prompt-flow gap`;
  - не запускать `5.4/5.5`;
  - шаг 8 пометить `NOT VERIFIED (upstream_plan_qna_unresolved)` и перейти к шагу 99.
...
- При вопросах: retry.
- После каждого run сохранить `05_review_spec_report_check_run<N>.txt`:
...
- Если итог `WARN/BLOCKED` из-за unresolved `Q*`:
  - повторить `/feature-dev-aidd:review-spec $TICKET` один раз.
- Если `prd_findings_sync_needed=1`, `open_questions_count=0`:
  - сохранить `05_prd_findings_sync_request.txt`;
  - выполнить один sync-retry `/feature-dev-aidd:idea-new $TICKET $IDEA_NOTE AIDD:SYNC_FROM_REVIEW ...`;
  - повторить `/feature-dev-aidd:review-spec $TICKET` один раз.
- Если `plan_findings_sync_needed=1`, `open_questions_count=0`:
  - сохранить `05_plan_findings_sync_request.txt`;
  - выполнить один sync-retry `/feature-dev-aidd:plan-new $TICKET AIDD:SYNC_FROM_REVIEW ...`;
  - повторить `/feature-dev-aidd:review-spec $TICKET` один раз.
- Если hang/kill — runtime probe:
...
- Запускать только при `readiness_gate=PASS` (см. 5.2.1).
- Перед first run проверить PRD header (`Status:` + unresolved `Q*`):
  - если `Status != READY` и последний `05_review_spec_report_check_run<N>.txt` показывает `prd_findings_sync_needed=1`, выполнить findings-sync через `idea-new`, затем повторить `review-spec`;
  - если `Status != READY` и последний `05_review_spec_report_check_run<N>.txt` показывает `plan_findings_sync_needed=1`, выполнить findings-sync через `plan-new`, затем повторить `review-spec`;
  - если после findings-sync `Status != READY`, классифицировать как `NOT VERIFIED (findings_sync_not_converged)` + `prompt-flow gap` и не запускать `tasks-new`.
- First run: `/feature-dev-aidd:tasks-new $TICKET`.
- При вопросах: retry.
- Если ответ содержит `upstream_blocker` / unresolved `Q*`:
  - перед retry проверить PRD header (`Status:` + unresolved `Q*`); если `Status != READY`, сначала выполнить findings-sync cycle (idea-new/plan-new), затем повторно проверить `Status`;
  - если после findings-sync `Status != READY`, классифицировать как `NOT VERIFIED (findings_sync_not_converged)` + `prompt-flow gap` и не выполнять retry;
...
- Precheck `AIDD:TEST_EXECUTION`:
  ...
  - если одна task-entry содержит `&&`, `||`, `;`, по умолчанию пометить `INFO(tasklist_hygiene)` и продолжить QA с фиксацией риска;
  - повышать до `WARN(tasklist_hygiene)` только если тот же shell-chain реально исполнялся в текущем QA run и дал fail-сигнал.
- Если в логе виден direct вызов non-canonical stage preflight runtime вне stage-chain:
...
- Сохранить `99_workspace_layout_check.txt` с проверкой, что в workspace root нет non-canonical root путей `docs/**|reports/**|config/**|.cache/**` вне `aidd/`.
