...
5. Если причина BLOCKED связана с отсутствующим spec (`aidd/docs/spec/<ticket>.spec.yaml`), unresolved PRD-вопросами (`Q*`) или `PRD Status != READY` (например `draft`):
   ...
   - если после `spec-interview + review-spec` PRD остаётся `Status != READY`, классифицируй как `prompt-flow gap (spec_sync_incomplete_prd_not_ready)` и не запускай stage, который требует `PRD READY`.
...
- перед первым запуском проверить PRD header (`Status:`): если не `READY` или есть unresolved `Q*`, сначала:
  ...
  - затем повторно проверить PRD header (`Status:` + unresolved `Q*`); если PRD всё ещё не `READY`, классифицировать как `prompt-flow gap (spec_sync_incomplete_prd_not_ready)`, пометить шаг 5.5 `NOT VERIFIED` и не запускать `tasks-new`.
- first run ticket-only;
- при вопросах: retry;
- если tasks-new сообщает `Missing Spec File` / `aidd/docs/spec/$TICKET.spec.yaml` / unresolved `Q*`:
  ...
  - выполнить `/feature-dev-aidd:spec-interview $TICKET` (ticket-only, при вопросах один retry);
  - перед retry снова проверить PRD header (`Status:` + unresolved `Q*`); если `Status != READY`, классифицировать как `prompt-flow gap (spec_sync_incomplete_prd_not_ready)` и не выполнять retry `tasks-new`;
