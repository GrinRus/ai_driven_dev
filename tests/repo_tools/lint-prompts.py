...
LEGACY_BASELINE_JSON_PRIMARY = Path("aidd/reports/migrations/commands_to_skills_frontmatter.json")
LEGACY_BASELINE_JSON_FALLBACK = Path("docs/migrations/commands_to_skills_frontmatter.json")
...
def lint_skills(root: Path) -> Tuple[List[str], List[str]]:
    ...
    baseline_candidates = [
        ...
        root / LEGACY_BASELINE_JSON_PRIMARY,
        root / LEGACY_BASELINE_JSON_FALLBACK,
    ]
...
def main() -> int:
    ...
    {info.front_matter.get("name", info.stem) for info in agent_files.values()}
