#!/usr/bin/env python3
"""Collect simple size stats for reports and optionally update AGENTS.md."""
...
def _render_table(rows: List[Tuple[str, int, int, int]]) -> str:
    if not rows:
        return "No reports found. Run `python3 tests/repo_tools/report_stats.py --write` after reports exist."
...
def main() -> int:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Update AGENTS.md between report-stats markers.",
    )
