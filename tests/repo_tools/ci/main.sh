...
run_repo_hygiene_guard() {
  if ! command -v python3 >/dev/null 2>&1; then
    warn "python3 not found; skipping repo hygiene guard"
    return
  fi
  if [[ ! -f "tests/repo_tools/repo_hygiene.py" ]]; then
    warn "tests/repo_tools/repo_hygiene.py missing; skipping"
    return
  fi
  log "running repo hygiene guard"
  if ! python3 tests/repo_tools/repo_hygiene.py; then
    err "repo hygiene guard failed"
    STATUS=1
  fi
}

run_docs_parity_guard() {
  if ! command -v python3 >/dev/null 2>&1; then
    warn "python3 not found; skipping docs parity guard"
    return
  fi
  if [[ ! -f "tests/repo_tools/docs_parity_guard.py" ]]; then
    warn "tests/repo_tools/docs_parity_guard.py missing; skipping"
    return
  fi
  log "running docs parity guard"
  if ! python3 tests/repo_tools/docs_parity_guard.py --root "${ROOT_DIR}"; then
    err "docs parity guard failed"
    STATUS=1
  fi
}
...
run_dist_manifest_check() {
  if ! command -v python3 >/dev/null 2>&1; then
    warn "python3 not found; skipping dist manifest check"
    return
  fi
  if [[ ! -f "tests/repo_tools/dist_manifest_check.py" ]]; then
    warn "tests/repo_tools/dist_manifest_check.py missing; skipping"
    return
  fi
  log "running dist manifest check"
  if ! python3 tests/repo_tools/dist_manifest_check.py --root "${ROOT_DIR}"; then
    err "dist manifest check failed"
    STATUS=1
  fi
}
