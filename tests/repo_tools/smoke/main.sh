...
assert_gate_exit() {
  ...
  local output rc
  set +e
  output="$(CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" "$PLUGIN_ROOT/hooks/gate_workflow.py" <<<"$PAYLOAD" 2>&1)"
  rc=$?
  set -e
...
for event in ("PreToolUse", "UserPromptSubmit", "Stop", "SubagentStop"):
    if event == "Stop":
        assert_has("gate_workflow.py", event)
        assert_has("gate_tests.py", event)
        assert_has("gate_qa.py", event)
        assert_has("format_and_test.py", event)
        assert_has("lint_deps.py", event)
    if event == "SubagentStop":
        assert_has("context_gc_stop.py", event)
    if event == "PreToolUse":
        assert_has("context_gc_pretooluse.py", event)
    if event == "UserPromptSubmit":
        assert_has("context_gc_userprompt.py", event)
PY
...
run_hook context_gc_precompact.py >/dev/null
run_hook context_gc_sessionstart.py >/dev/null
run_hook context_gc_pretooluse.py >/dev/null
run_hook context_gc_userprompt.py >/dev/null
...
fmt_first="$("$PLUGIN_ROOT/hooks/format_and_test.py" 2>&1)"
...
fmt_second="$("$PLUGIN_ROOT/hooks/format_and_test.py" 2>&1)"
echo "$fmt_second" | grep -q "Dedupe: тесты уже запускались" || {
  echo "[smoke] dedupe did not skip repeated run" >&2
  echo "$fmt_second" >&2
  exit 1
}
fmt_force="$(AIDD_TEST_FORCE=1 "$PLUGIN_ROOT/hooks/format_and_test.py" 2>&1)"
...
JSON
fmt_common="$("$PLUGIN_ROOT/hooks/format_and_test.py" 2>&1)"
...
set +e
bad_output="$(cd "$BAD_DIR" && CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" "$PLUGIN_ROOT/hooks/gate_workflow.py" <<<"$PAYLOAD" 2>&1)"
bad_rc=$?
set -e
if [[ "$bad_rc" -eq 0 ]]; then
