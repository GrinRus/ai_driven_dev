...
def test_plugin_hooks_include_tests_and_post_hooks():
    ...
    assert any("format-and-test.sh" in cmd for cmd in post_cmds + stop_cmds + sub_stop_cmds), (
        "format-and-test missing in PostToolUse/Stop/SubagentStop"
    )
    assert any("lint-deps.sh" in cmd for cmd in post_cmds + stop_cmds + sub_stop_cmds), (
        "lint-deps missing in PostToolUse/Stop/SubagentStop"
    )
