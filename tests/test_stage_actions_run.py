...
class StageActionsRunTests(unittest.TestCase):
    ...
    def test_main_emits_blocked_stage_result_when_run_fails_without_stage_result(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage-actions-run-") as tmpdir:
            root = Path(tmpdir)
            context = launcher.LaunchContext(
                root=root,
            ...
            launch_result = launcher.LaunchResult(
                ...
                wrapped_exit_code=1,
                stdout="",
                stderr="[aidd] ERROR: reason_code=contract_mismatch_actions_shape\n",
                log_path=root / "reports" / "logs" / "wrapper.log",
                output_limited=False,
                stdout_lines=0,
                stdout_bytes=0,
            ...
            with (
                ...
                patch("aidd_runtime.stage_actions_run.launcher.log_path", return_value=launch_result.log_path),
                patch("aidd_runtime.stage_actions_run.launcher.run_guarded", return_value=launch_result),
                patch("aidd_runtime.stage_actions_run._stage_result_mtime_ns", side_effect=[-1, -1]),
                patch("aidd_runtime.stage_actions_run.stage_result_runtime.main", return_value=0) as stage_result_main,
            ):
                code = stage_actions_run.main([], default_stage="implement", description="test")
            ...
            stage_result_main.assert_called_once()
            emitted_args = stage_result_main.call_args.args[0]
            self.assertIn("--result", emitted_args)
            self.assertIn("blocked", emitted_args)
            self.assertIn("--reason-code", emitted_args)
            self.assertIn("contract_mismatch_actions_shape", emitted_args)
            self.assertIn("--error", emitted_args)
            self.assertIn("error_during_execution", emitted_args)

    def test_main_skips_fallback_emit_when_stage_result_already_updated(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage-actions-run-") as tmpdir:
            root = Path(tmpdir)
            context = launcher.LaunchContext(
                root=root,
            ...
            launch_result = launcher.LaunchResult(
                ...
                wrapped_exit_code=1,
                stdout="",
                stderr="[aidd] ERROR: reason_code=seed_scope_cascade_detected\n",
                log_path=root / "reports" / "logs" / "wrapper.log",
                output_limited=False,
                stdout_lines=0,
                stdout_bytes=0,
            ...
            with (
                ...
                patch("aidd_runtime.stage_actions_run.launcher.log_path", return_value=launch_result.log_path),
                patch("aidd_runtime.stage_actions_run.launcher.run_guarded", return_value=launch_result),
                patch("aidd_runtime.stage_actions_run._stage_result_mtime_ns", side_effect=[-1, 123]),
                patch("aidd_runtime.stage_actions_run.stage_result_runtime.main", return_value=0) as stage_result_main,
            ):
                code = stage_actions_run.main([], default_stage="review", description="test")

            self.assertEqual(code, 1)
            stage_result_main.assert_not_called()
