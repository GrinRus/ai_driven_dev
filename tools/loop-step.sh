...
def _bootstrap() -> None:
    ...
    if used_auto:
        print(f"[loop-step] WARN: CLAUDE_PLUGIN_ROOT not set; auto-detected {plugin_root}", file=sys.stderr)
...
def main() -> int:
    try:
        ...
        print(f"[loop-step] ERROR: {exc}", file=sys.stderr)
        return 2
    from tools import loop_step as tools_module
