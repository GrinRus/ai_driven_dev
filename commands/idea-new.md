...
allowed-tools:
  ...
  - "Bash(PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli analyst-check:*)"
model: inherit
...
- `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli set-active-feature` синхронизирует `.active_*` и scaffold'ит PRD.
- `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli set-active-stage idea` фиксирует стадию `idea`.
- Команда должна запускать саб-агента **analyst** (Claude: Run agent → analyst).
- `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli analyst-check --ticket <ticket>` — проверка диалога/статуса после ответов.
...
## Пошаговый план
1. Зафиксируй стадию `idea`: `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli set-active-stage idea`.
2. Обнови активный тикет/slug: `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli set-active-feature "$1" [--slug-note "$2"]`.
