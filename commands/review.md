...
## Контекст
Команда `/feature-dev-aidd:review` запускает саб-агента **feature-dev-aidd:reviewer** для проверки изменений перед QA и фиксирует замечания в tasklist. После ревью создаётся отчёт и формируются handoff‑задачи в `AIDD:HANDOFF_INBOX`. Свободный ввод после тикета используй как контекст ревью.
...
- Команда должна запускать саб-агента **feature-dev-aidd:reviewer** (Claude: Run agent → feature-dev-aidd:reviewer).
