...
6. Read order after stage-chain preflight artifacts: `readmap.md` -> loop pack -> review pack (if exists) -> memory semantic/decisions packs -> stage memory slice manifest (`<ticket>-memory-slices.review.<scope_key>.pack.json`) -> rolling context pack; use `rg` only as controlled fallback after memory slice manifest.
