...
usage() {
  cat <<'EOU'
Usage: context_expand.sh --path <path[#AIDD:...|@handoff:...]> --reason-code <code> --reason <text> [--expand-write] [--no-regenerate-pack] [--ticket <ticket>] [--scope-key <scope>] [--work-item-key <work_item>] [--stage <stage>]
EOU
}
...
main() {
  local cmd=(
    "${CLAUDE_PLUGIN_ROOT}/tools/context-expand.sh"
    --ticket "$AIDD_TICKET"
