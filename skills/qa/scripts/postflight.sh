...
usage() {
  cat <<'EOF'
Usage: postflight.sh --ticket <ticket> --scope-key <scope> --work-item-key <work_item> --stage <stage> [--actions <path>] [--result <continue|done|blocked>]
EOF
}
...
main() {
...

  {
    ...
    "${CLAUDE_PLUGIN_ROOT}/skills/aidd-core/scripts/progress.sh" --ticket "$AIDD_TICKET" --source qa
