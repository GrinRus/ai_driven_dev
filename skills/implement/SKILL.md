...
6. Read order after stage-chain preflight artifacts: `readmap.md` -> loop pack -> review pack (if exists) -> memory semantic/decisions packs -> stage memory slice manifest (`<ticket>-memory-slices.implement.<scope_key>.pack.json`) -> rolling context pack; use `rg` only as controlled fallback after memory slice manifest.
...
9. Canonical stage-chain: preflight (`preflight_prepare.py` inside stage-chain) -> stage runtime -> actions_apply.py/postflight -> `python3 ${CLAUDE_PLUGIN_ROOT}/skills/aidd-flow-state/runtime/stage_result.py`; it must produce `aidd/reports/loops/<ticket>/<scope_key>/stage.implement.result.json`. `[AIDD_LOOP_POLICY:CANONICAL_STAGE_RESULT_PATH]`
