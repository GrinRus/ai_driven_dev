...
# Workflow (idea → plan → validation → tasks → implementation → review)

## 1. Идея и PRD
- Запустите `/feature-activate <slug>` и `/idea-new <slug> [TICKET]`.
- Агент analyst задаёт вопросы и формирует PRD по `docs/prd.template.md`.
- В конце фиксируем статус READY/BLOCKED и список открытых рисков.

## 2. Планирование и валидация
- `/plan-new <slug>` вызывает planner для построения дорожной карты (итерации, координация, DoD).
- После генерации автоматически подключается validator: он проверяет полноту и формирует вопросы.
- Все блокеры отражаем в PRD и плане.

## 3. Декомпозиция
- `/tasks-new <slug>` обновляет `tasklist.md`, синхронизируя этапы разработки, QA и документации.
- Требуется указать зависимости, критерии приёмки и ссылки на артефакты.

## 4. Реализация
- `/implement <slug>` подключает implementer: он следует плану, коммитит малыми шагами и перезапускает `/test-changed`.
- Гейты блокируют работу с `src/**`, пока не готовы PRD/план/тасклист.
- `/api-spec-new`, `/tests-generate`, `/docs-generate` — дополнительные команды при необходимости.

## 5. Код-ревью
- `/review <slug>` вызывает reviewer, который проверяет diff, тесты и соответствие плану.
- Замечания фиксируются в `tasklist.md`; статус BLOCKED возвращает фичу implementer'у.

## 6. Завершение
- Убедитесь, что чеклист в `tasklist.md` закрыт.
- Выполните `/commit-validate`, `/conventions-sync` и обновите `docs/release-notes.md` при релизе.
MD
