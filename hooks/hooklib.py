...
_EVENT_DEDUP_TYPES = {"gate-tests", "format-and-test"}
...
def append_event(
    ...
    should_dedupe = _event_should_dedupe(payload, dedup_window)
    if should_dedupe:
...
def _write_event_line(path: Path, payload: Dict[str, Any]) -> None:
    try:
...
def _event_dedup_scope_key(payload: Dict[str, Any]) -> str:
...
def _event_should_dedupe(payload: Dict[str, Any], window_seconds: int) -> bool:
    _ = window_seconds
    event_type = str(payload.get("type") or "").strip().lower()
    status = str(payload.get("status") or "").strip().lower()
    return event_type in _EVENT_DEDUP_TYPES and status in _EVENT_DEDUP_STATUSES
...
def _event_dedup_duplicate_state(path: Path, payload: Dict[str, Any], window_seconds: int) -> tuple[bool, bool]:
    if not path.exists():
        return False, False
    try:
        ...
        return False, False
    dedup_key = _event_dedup_key(payload)
    now = _parse_iso_ts(str(payload.get("ts") or ""))
    if now is None:
