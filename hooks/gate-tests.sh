...
try:
    ...
    print(
        f"WARN: reviewer маркер повреждён ({marker_path}). Пересоздайте его командой `PYTHONPATH=${{CLAUDE_PLUGIN_ROOT:-.}} python3 -m aidd_runtime.cli reviewer-tests --status required`."
    )
    raise SystemExit(0)
...
label = ticket if not slug_hint or slug_hint == ticket else f"{ticket} (slug hint: {slug_hint})"
print(f"WARN: {file_path} не входит в список Researcher targets → обновите PYTHONPATH=${{CLAUDE_PLUGIN_ROOT:-.}} python3 -m aidd_runtime.cli research для {label} или настройте paths.")
PY
)"
  if [[ -n "$message" ]]; then
    log_stdout "$message"
