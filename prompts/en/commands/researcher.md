...
## Automation & Hooks
- `claude-workflow research --ticket <ticket> --auto` (plus `--paths/--keywords/--note` options) populates `reports/research/<ticket>-context.json`.
- `--dry-run` writes only JSON; `--targets-only` refreshes target paths without scanning; `--no-agent` skips launching the Claude agent.
...
2. Execute `claude-workflow research --ticket "$1" --auto [extra options]`.
3. If no matches are found, scaffold `docs/research/$1.md` and mark the baseline.
4. Launch **researcher** with the generated JSON, fill patterns/anti-patterns, absence of patterns, notes.
...
## CLI Examples
- `/researcher ABC-123 --paths src/service:src/lib --keywords "checkout,pay"`
- `!bash -lc 'claude-workflow research --ticket "ABC-123" --auto --note "Reuse checkout facade"'`
