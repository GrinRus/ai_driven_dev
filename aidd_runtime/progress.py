    ...
    *,
    ...
    if context == "handoff":
        guidance = (
            ...
            f"в {tasklist_rel} и повторите `PYTHONPATH=${{CLAUDE_PLUGIN_ROOT:-.}} "
            f"python3 -m aidd_runtime.cli progress --source handoff --ticket {ticket}`."
        )
    else:
        guidance = (
            f"BLOCK: в фиче `{ticket}` есть изменения в коде ({summary}), "
            ...
            "обновите строку `Checkbox updated: …` и повторите `PYTHONPATH="
            "${{CLAUDE_PLUGIN_ROOT:-.}} python3 -m aidd_runtime.cli progress --source "
