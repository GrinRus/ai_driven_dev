...
try:
    from ._bootstrap import ensure_repo_root, resolve_repo_root
...
HOOK_PATTERN = re.compile(r"(?:\$\{CLAUDE_PLUGIN_ROOT\}/)?hooks/([A-Za-z0-9_.-]+\.sh)")

CANONICAL_EXEC_RE = re.compile(
    r'exec\s+"?\$\{CLAUDE_PLUGIN_ROOT\}/(skills/[A-Za-z0-9_.-]+/runtime/[A-Za-z0-9_.-]+\.py)"?'
)
SCRIPT_REF_RE = re.compile(
    r"(?:\$\{CLAUDE_PLUGIN_ROOT\}/)?((?:skills/[A-Za-z0-9_.-]+/runtime/[A-Za-z0-9_.-]+\.py)|(?:hooks/[A-Za-z0-9_.-]+\.sh)|(?:tools/[A-Za-z0-9_.-]+\.(?:sh|py)))"
)
PYTHON_CALL_RE = re.compile(
    r"\bpython(?:3)?\s+(?:\"|')?\$\{CLAUDE_PLUGIN_ROOT\}/([A-Za-z0-9_./-]+\.py)(?:\"|')?"
)
