...
- CLI-команда `claude-workflow research --ticket <ticket> --auto --deep-code --call-graph [--reuse-only] [--paths/--keywords/--langs/--graph-langs/--note]` собирает контекст: пути из `config/conventions.json`, `code_index` (символы/импорты/тесты), `reuse_candidates` и `call_graph`/`import_graph` (только Java/Kotlin через tree-sitter при наличии). Результат сохраняется в `reports/research/<ticket>-targets.json` и `<ticket>-context.json`.
