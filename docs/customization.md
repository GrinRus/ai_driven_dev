...
- `prd_review` — описывает, как проверяется раздел `## PRD Review`: список веток, допустимые статусы (`approved`), уровни блокировки (`blocked`) и команда запуска агента (`command`).
