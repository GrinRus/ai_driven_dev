...
- `prd_review` — описывает, как проверяется раздел `## PRD Review`: ветки, допустимые (`approved`) и блокирующие (`blocked`) статусы, `allow_missing_report` (разрешить отсутствие отчёта `reports/prd/{slug}.json`), `blocking_severities` (уровни findings, которые блокируют), `report_path` для кастомизации расположения отчёта.
...
```markdown
```command
!`claude-workflow preset feature-design --feature "$1"`
```
```
