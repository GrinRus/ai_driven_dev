...
Planned scope only: paths in this section describe future implementation and
may be absent in the current repository tree until Wave 122 is delivered.
...
Planned scope only: integration targets below are roadmap work items, not a
claim that every referenced file already contains memory-v2 wiring today.
