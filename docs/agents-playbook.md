...
> `contract-checker` и `db-migrator` не имеют отдельных слэш-команд. Вызывайте их из палитры `Claude: Run agent …` в IDE или через `/docs-generate` с указанием имени агента (`/docs-generate contract-checker <slug>`), если нужно принудительно сверить контракт/миграцию.
