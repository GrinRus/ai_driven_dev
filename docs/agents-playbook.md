...
| --- | --- | --- | --- | --- | --- |
| 1 | `/idea-new <slug> [TICKET]` | `analyst` | Свободная идея, вводные | `docs/.active_feature`, `docs/prd/<slug>.prd.md`, статус READY/BLOCKED | PRD заполнен, slug обновлён и открытые вопросы выписаны |
...
1. Создайте ветку `git checkout -b feature/<TICKET>` (или аналогичный пресет из `config/conventions.json`).
2. `/idea-new <slug> [TICKET]` — команда зафиксирует slug в `docs/.active_feature`, подготовит PRD и список вопросов аналитика.
3. `/plan-new <slug>` → закройте вопросы валидатора до статуса PASS.
4. `/tasks-new <slug>` → проверьте, что чеклисты отражают план.
5. `/api-spec-new <slug>` и `contract-checker` — согласуйте контракт с кодом.
6. `db-migrator` (если затрагиваются данные) и `/tests-generate <slug>` — подготовьте миграции и тесты заранее.
7. `/implement <slug>` → двигайтесь малыми итерациями, следите за гейтами.
8. `/review <slug>` → запросите финальное ревью после прохождения всех барьеров.
