...
Статус: closed (implemented on branch `codex/implement-seed-nonconverging-fix`). Основание — incident-driven fix plan для TST-001 (step 7/8 terminal FAIL + non-deterministic recovery paths).
...
Статус: in-progress (partial closure via Wave 122). Основание — результаты quality e2e run 20260330T081411Z по тикету TST-002; цель — повысить качество кода и артефактов, генерируемых AIDD.
...
- [x] **W120-1 (P0) Enforce bounded terminalization for plan-new stream runs** `tests/repo_tools/aidd_stage_launcher.py`, `docs/e2e/aidd_test_flow_prompt_ralph_script_full.txt`, `skills/plan-new/SKILL.md`:
  ...
  - **Closed (2026-03-31):** superseded by `W122-5`; bounded terminalization and classifiable synthetic top-level result are enforced in launcher/runner (`d5e229f`, `6120c46`, `45fe346`).
...
- [x] **W120-3 (P2) Improve runner convergence diagnostics for verbose stream-json sessions** `tests/repo_tools/aidd_stage_launcher.py`, `docs/e2e/aidd_test_flow_prompt_ralph_script_full.txt`:
  ...
  - **Closed (2026-03-31):** superseded by `W122-5`; convergence telemetry + deterministic terminal subtype (`prompt_budget_exhausted`) now emitted by canonical audit surfaces (`d5e229f`, `6120c46`).
...
Статус: in-progress (partial closure via Wave 122). Основание — результаты quality e2e run 20260330T073513Z по тикету TST-002; цель — повысить качество кода и артефактов, генерируемых AIDD.
...
- [x] **W119-2 (P1) Add bounded terminalization for preflight healthcheck no-convergence** `tests/repo_tools/aidd_stage_launcher.py`, `tests/repo_tools/aidd_audit_runner.py`, `docs/e2e/aidd_test_flow_prompt_ralph_script_full.txt`:
  ...
  - **Closed (2026-03-31):** superseded by `W122-5`; preflight/seed no-convergence now goes through deterministic terminalization/classification path without manual kill (`d5e229f`, `6120c46`).
...
- [x] **W117-1 (P0) Enforce terminal stage result emission for idea-new** `skills/idea-new/SKILL.md`, `tests/repo_tools/aidd_audit_runner.py`, `tests/repo_tools/aidd_stage_launcher.py`:
  ...
  - **Closed (2026-03-31):** подтверждено Wave 121 доработками launcher/runner (`2d41796`, `45fe346`) и regression-сценарием `external_143_no_kill` для `05_idea_new` без `no_top_level_result`.
...
## P0-now — Risk-first queue (2026-03-31, post-branch verification cleanup)

1. `W119-1` — remove retired wrapper dependency from `status` healthcheck path.
2. `W118-2` — implement-stage deterministic test command synthesis/retry fingerprinting.
3. `W113-24` — strict canonical `aidd/*` workspace layout enforcement (no root migration).
4. `W114-2` — plugin write-safety inconclusive classification refinement.
5. `W113-8` — `review-spec` narrative/report parity with report payload as SoT.
6. `W113-1` — follow-up mismatch guard after `W113-8`.
7. `W113-3` — implement-stage cross-source derive drift prevention.
8. `W113-4` — review-stage explicit `tasks_derive --source review` contract hardening.
9. `W114-3` — QA derive flow pinning to `--source qa`.
10. `W114-4` — researcher surface minimization (remove wildcard derive exposure).
...
- [x] **W114-1 (P0-now) Harden stream-path telemetry when CLI does not emit stream path** `tests/repo_tools/aidd_stream_paths.py`, `tests/repo_tools/aidd_audit_runner.py`, `tests/repo_tools/test_aidd_stage_launcher.py`, `tests/repo_tools/test_aidd_audit_runner.py`:
  ...
  - **Closed (2026-03-31):** superseded by `W122-5` + `W113-27` closure; anomaly marker `stream_path_not_emitted_by_cli` and deterministic INFO classification validated in audit runner regressions (`d5e229f`, `6120c46`).
...
- [x] **W113-9 (P0-now) Workspace-layout classifier baseline awareness (pre-existing root paths)** `tests/repo_tools/aidd_audit_runner.py`, `tests/repo_tools/test_aidd_audit_runner.py`, `tests/repo_tools/test_e2e_prompt_contract.py`:
  ...
  - **Closed (2026-03-31):** подтверждено в текущей ветке regression-покрытием `test_workspace_layout_preexisting_paths_report_info_not_warn`, `test_workspace_layout_delta_reports_warn`, replay fixture `20260317`.
...
- [x] **W113-26 (P0-now) Write-safety classifier baseline awareness for root non-canonical paths** `tests/repo_tools/aidd_audit_runner.py`, `tests/repo_tools/test_e2e_prompt_contract.py`, `tests/repo_tools/test_audit_runner.py`:
  ...
  - **Closed (2026-03-31):** закрыто в том же изменении, что `W113-9`; пункт признан дублирующим по AC/рискам, сохранён для traceability и снят из `P0-now`.
...
- [x] **W113-27 (P0-now) Stream-path telemetry completeness for stall classification** `tests/repo_tools/aidd_audit_runner.py`, `tests/repo_tools/e2e_prompt/profile_full.md`, `tests/repo_tools/test_e2e_prompt_contract.py`:
  ...
  - **Closed (2026-03-31):** superseded by `W122-5`; fallback/noise handling consolidated in launcher+audit classification (`d5e229f`, `6120c46`) with regression coverage in `test_aidd_audit_runner.py`.
