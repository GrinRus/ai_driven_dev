...
_Статус: done (local implementation). Основание — TST-001 показал системный optimism drift между `tasklist`, `.active`, `index`, `status` и фактическими downstream artifacts: tasklist рекламирует reviewer/qa/tests outputs как будто они уже существуют, derived state смешивает expected и actual evidence, а repeated `gate-tests warn` делает status/evidence слой шумным. Research stage сознательно исключён из этой волны и остаётся отдельным redesign track._

- [x] **W148-1 (P0) Shared non-research artifact truth evaluator + soft policy knob** `skills/aidd-core/runtime/artifact_truth.py`, `templates/aidd/config/gates.json`, `tests/helpers.py`, `tests/test_index_schema.py`, `tests/test_tasklist_check.py`:
  - ввести общий evaluator для `PRD|plan|spec|tasklist|.active|reports/**`, который вычисляет `doc_statuses`, `expected_reports`, `actual_reports`, `missing_expected_reports`, `truth_checks`;
  - добавить policy section `artifact_truth` (`mode=soft|hard`, `collapse_event_noise`, `warn_on_missing_expected_reports`, `warn_on_stage_doc_drift`);
  - hard mode пока compatibility surface only, default остаётся `soft`.
  **AC:** runtime surfaces могут посчитать truth picture без обращения к research semantics; отсутствие `artifact_truth` в legacy workspace трактуется как soft-default.
  **Deps:** -
  **Regression/tests:** `python3 -m pytest -q tests/test_index_schema.py tests/test_tasklist_check.py`.
  **Effort:** M
  **Risk:** Medium
  **Closure note (2026-04-16):** added shared `artifact_truth` evaluator, soft-default `artifact_truth` config section, and regression coverage for truth fields / soft warnings.

- [x] **W148-2 (P0) Separate expected vs actual report contracts in tasklist/index/status** `skills/tasks-new/templates/tasklist.template.md`, `skills/tasks-new/runtime/tasks_new.py`, `skills/status/runtime/index_sync.py`, `skills/status/runtime/status.py`, `skills/aidd-core/templates/index.schema.json`, `tests/test_tasks_new_runtime.py`, `tests/test_status.py`, `tests/test_index_schema.py`:
  - перевести writer contract tasklist с `Reports:` на `ExpectedReports:`; legacy `Reports:` читать как alias expected-surface и мигрировать при `tasks-new`;
  - оставить `index.reports` только для реально существующих outputs, а expected paths публиковать отдельно;
  - показать в status различие между `Reports (actual)`, `Expected reports (planned)` и `Missing expected reports`.
  **AC:** tasklist больше не выглядит authoritative источником факта для reviewer/qa/tests до появления файлов; old workspaces с `Reports:` не ломаются.
  **Deps:** W148-1
  **Regression/tests:** `python3 -m pytest -q tests/test_tasks_new_runtime.py tests/test_status.py tests/test_index_schema.py`.
  **Effort:** M
  **Risk:** Medium
  **Closure note (2026-04-16):** tasklist template now emits `ExpectedReports`, `tasks-new` migrates legacy `Reports`, index keeps `reports` as actual-only, and status surfaces show planned vs actual separately.

- [x] **W148-3 (P1) Truth warnings in tasklist validation + event de-noise in consumer surfaces** `skills/aidd-flow-state/runtime/tasklist_validate.py`, `skills/status/runtime/index_sync.py`, `skills/status/runtime/status.py`, `tests/test_tasklist_check.py`, `tests/test_index_schema.py`, `tests/test_status.py`:
  - публиковать `stage_doc_drift`, `missing_expected_report`, `tasklist_ready_without_actual_downstream_reports`, `active_stage_vs_plan_mismatch`, `stale_reference`, `test_execution_contract_weak` как soft warnings по policy;
  - схлопывать repeated identical `gate-tests warn` в summary-entry (`repeat_count`, `first_seen`, `last_seen`, `sample_reason`) только в derived/read surfaces, без изменения raw JSONL.
  **AC:** status/index становятся пригодны для чтения без потери audit fidelity; новые truth checks не создают terminal blockers по умолчанию.
  **Deps:** W148-1, W148-2
  **Regression/tests:** `python3 -m pytest -q tests/test_tasklist_check.py tests/test_index_schema.py tests/test_status.py tests/test_gate_tests_hook.py`.
  **Effort:** M
  **Risk:** Medium
  **Closure note (2026-04-16):** validator now emits soft truth warnings, index/status collapse repeated `gate-tests warn`, and raw event JSONL stays unchanged.

- [x] **W148-4 (P2) Prompt/runtime wording cleanup around expected vs actual outputs** `skills/tasks-new/templates/tasklist.template.md`, `CHANGELOG.md`, `AGENTS.md`:
  - обновить wording tasklist/template surfaces так, чтобы expected artifacts не выглядели как уже существующие evidence;
  - зафиксировать contract shift в release notes/dev docs.
  **AC:** canonical template и docs явно разделяют planned outputs и actual evidence.
  **Deps:** W148-2
  **Regression/tests:** `python3 tests/repo_tools/lint-prompts.py --root .`.
  **Effort:** S
  **Risk:** Low
  **Closure note (2026-04-16):** template wording, changelog and repo guide now explicitly separate planned `ExpectedReports` from actual evidence in `aidd/reports/**`.
