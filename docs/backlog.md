...
1. `W120-1` — bounded terminalization for `plan-new` in long stream sessions.
2. `W119-1` — remove retired wrapper dependency from `status` healthcheck path.
3. `W119-2` — deterministic preflight healthcheck terminalization without operator kill.
4. `W118-2` — implement-stage deterministic test command synthesis/retry fingerprinting.
5. `W113-24` — strict canonical `aidd/*` workspace layout enforcement (no root migration).
6. `W114-1` — stream-path telemetry hardening when CLI omits path emission.
7. `W113-27` — stream-path fallback completeness for stable stall classification.
8. `W114-2` — plugin write-safety inconclusive classification refinement.
9. `W113-8` — `review-spec` narrative/report parity with report payload as SoT.
10. `W113-1` — follow-up mismatch guard after `W113-8`.
11. `W113-3` — implement-stage cross-source derive drift prevention.
12. `W113-4` — review-stage explicit `tasks_derive --source review` contract hardening.
13. `W114-3` — QA derive flow pinning to `--source qa`.
14. `W114-4` — researcher surface minimization (remove wildcard derive exposure).
