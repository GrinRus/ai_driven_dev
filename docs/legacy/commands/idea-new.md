...
- `aidd/docs/.active.json` — активные маркеры.
...
- `${CLAUDE_PLUGIN_ROOT}/tools/set-active-feature.sh` синхронизирует `.active.json` и scaffold'ит PRD.
...
- Заполни поля stage/agent/read_next/artefact_links/what-to-do под idea.
...
4. Команда → subagent: **Use the feature-dev-aidd:analyst subagent. First action: Read `aidd/reports/context/$1.pack.md`.**
...
## Ожидаемый вывод
- Активный ticket/slug зафиксирован в `aidd/docs/.active.json`.
