...
## Пошаговый план
1. Запусти `python3 tools/set_active_feature.py "$1" [--slug-note "$2"]` — команда обновит `docs/.active_ticket`, `.active_feature` и создаст PRD (`Status: draft`). Аргумент `--force` используем только если подтверждено перезаписывание существующей фичи.
...
4. Запусти саб-агента **analyst** через `/analyst` (или палитру) и попроси его сначала собрать данные из `doc/backlog.md`, `docs/research/<ticket>.md`, `reports/research/*.json`. Вопросы пользователю разрешены только для пробелов; ответ должен приходить в формате `Ответ N: …`.
