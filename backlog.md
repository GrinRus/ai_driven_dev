...
_Решение: legacy-root (workspace без `aidd/`) не поддерживаем; `AIDD_ROOT` — строгий root без авто‑`/aidd`._
- [x] Ввести единый helper `hook_project_root` в `src/claude_workflow_cli/data/payload/aidd/hooks/lib.sh` с приоритетом: `AIDD_ROOT` → `CLAUDE_PROJECT_DIR` → `CLAUDE_PLUGIN_ROOT` → `git -C "$PWD" rev-parse --show-toplevel` → поиск вверх `aidd/docs`; возвращать абсолютный путь (realpath) и пустое значение при провале.
- [x] Привести все hook‑скрипты к одному резолверу и абсолютным путям (без `cd`): `src/claude_workflow_cli/data/payload/aidd/hooks/{gate-workflow.sh,gate-prd-review.sh,gate-tests.sh,gate-qa.sh,lint-deps.sh,format-and-test.sh}`; все git‑команды только через `git -C "$ROOT_DIR" ...`; если root не найден — мягкий exit без блокировки.
- [x] Вынести общий Python‑resolver (например, `src/claude_workflow_cli/paths.py`) и заменить вызовы в `src/claude_workflow_cli/feature_ids.py`, `src/claude_workflow_cli/context_gc/hooklib.py`, `src/claude_workflow_cli/tools/{qa_agent.py,prd_review.py,prd_review_gate.py,plan_review_gate.py,research_guard.py,analyst_guard.py}` и в Python‑хуке `src/claude_workflow_cli/data/payload/aidd/hooks/format-and-test.sh`; исключить относительные `./aidd` пути.
- [x] Обновить документацию об источнике root и запуске CLI/хуков: `src/claude_workflow_cli/data/payload/aidd/docs/sdlc-flow.md`, `src/claude_workflow_cli/data/payload/aidd/conventions.md`, `src/claude_workflow_cli/data/payload/aidd/commands/implement.md`, `src/claude_workflow_cli/data/payload/aidd/agents/implementer.md`, `src/claude_workflow_cli/data/payload/aidd/docs/tasklist/template.md` (правило: хуки и CLI работают от `aidd/`; `AIDD_ROOT` как override).
- [x] Добавить тесты на резолвинг root: `AIDD_ROOT` override, конкуренция `CLAUDE_PROJECT_DIR`/`CLAUDE_PLUGIN_ROOT`, запуск hook из чужого cwd, non‑git → мягкий exit, игнор legacy‑docs при наличии `aidd/`. Файлы: `tests/test_feature_ids_root.py`, `tests/test_gate_workflow.py`, `tests/test_gate_qa.py`, `tests/test_gate_tests_hook.py`, `tests/test_post_hook_paths.py`, `tests/helpers.py`.
- [x] Обновить негативный кейс в `src/claude_workflow_cli/data/payload/smoke-workflow.sh`: отсутствие root должно приводить к мягкому exit (без блокировки).
- [x] После унификации: обновить `src/claude_workflow_cli/data/payload/manifest.json`, прогнать `python3 tools/check_payload_sync.py`, синхронизировать root через `scripts/sync-payload.sh --direction=to-root`.
