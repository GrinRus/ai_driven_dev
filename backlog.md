...
- [x] Обновить `tasklist-check` и lint: требовать `AIDD:TEST_EXECUTION` + базовые поля (profile/tasks/filters/when/reason). **AC:** lint и check валидируют новую секцию. Файлы: `src/claude_workflow_cli/tools/tasklist_check.py`, `scripts/lint-prompts.py`, `tests/test_prompt_lint.py`.
