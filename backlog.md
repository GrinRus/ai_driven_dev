...
- [x] W78-12 `hooks/gate-stage-writes.sh`, `hooks/hooks.json`: запрет правок вне `aidd/**` на стадиях review/qa/spec-interview/tasklist, **per-stage allowlist**: review/qa → `aidd/docs/tasklist/**`, `aidd/reports/**`, `aidd/docs/.active_*`; spec-interview → `aidd/docs/spec/**`, `aidd/reports/**`, `aidd/docs/.active_*`; tasklist → `aidd/docs/tasklist/**`, `aidd/reports/**`, `aidd/docs/.active_*`. **AC:** gate запускается на тех же событиях, что и `gate-workflow` (min: Stop/SubagentStop), stage берётся из `.active_stage` или env override; не ломает `set-active-stage/set-active-feature`, spec-interview может писать spec. **Tests:** `tests/test_gate_workflow.py`, `tests/test_gate_qa.py` (включая `.active_*` allowlist). Deps: -
...
- [x] W78-27 `tests/test_gate_stage_writes.py` (или расширение существующих): покрыть allowlist/denylist для стадий review/qa/spec-interview/tasklist (включая `.active_*`). **DoD:** по 2 allow + 2 deny кейса на stage. **AC:** guard не ломает spec/tasklist. **Tests:** `tests/test_gate_stage_writes.py`. Deps: W78-12.
