...
- `PYTHONPATH=${CLAUDE_PLUGIN_ROOT:-.} python3 -m aidd_runtime.cli research-check` to validate research artefacts before planning.
...
- `analyst-check` no longer validates research; research validation is handled by `aidd_runtime.cli research-check` and `gate-workflow`.
- `/plan-new` now runs `aidd_runtime.cli research-check` before planning.
